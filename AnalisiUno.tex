...
Per ogni $x,y\in \RR$, $y\neq 0$ e per ogni $n,m \in \NN$
valgono le seguenti proprietà:
\begin{enumerate}
  \item  $x^{n+m} = x^n \cdot x^m$,
  \item $(x^n)^m = x^{n\cdot m}$,
  \item $(x\cdot y)^n = x^n \cdot y^n$,
  \item $\displaystyle \enclose{\frac{x}{y}}^n = \frac{x^n}{y^n}$.
\end{enumerate}
...
Per $a>1$ definiamo $\exp_a \colon \RR \to\RR$ come
\[
\exp_a(x) = \sup \left\{\sqrt[q]{a^p}\colon p\in \ZZ ,q\in \NN,\ q\neq 0,\ \frac{p}{q} < x\right\}.
\]
