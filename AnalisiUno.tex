...
Se la serie $\sum a_k$ è convergente
allora anche la serie $\sum b_n$ è convergente e
\[
\sum_{n=0}^\infty b_n
= \sum_{k=0}^\infty a_k.
\]
...
\[
m_{jk}  = \frac{z^k}{k!} \cdot \frac{w^j}{j!}.
\]
Se sommiamo tutti gli elementi nel quadrato
$0 \le j,k \le N$ e facciamo
tendere $N\to +\infty$ si ottiene
\[
  \sum_{k=0}^N \sum_{j=0}^N \frac{z^k}{k!} \frac{w^j}{j!}
  = \sum_{k=0}^N \frac{z^k}{k!} \sum_{j=0}^N \frac{w^k}{j!}
  \to \exp(z) \cdot \exp(w).
\]
Se invece sommiamo gli elementi nel triangolo
$0\le k+j \le N$ e facciamo tendere $N\to +\infty$
si ottiene
\[
\sum_{n=0}^N \sum_{k=0}^n \frac{z^k}{k!} \frac{w^{n-k}}{(n-k)!}
= \sum_{n=0}^N \sum_{k=0}^n {n \choose k} \frac{z^k w^{n-k}}{n!}
= \sum_{n=0}^N \frac{(z+w)^n}{n!}
\to \exp(z+w).
\]
Per concludere la dimostrazione è sufficiente notare
che i due diversi modi di sommare gli elementi della
matrice $m_{jk}$ corrispondono a due diverse sotto


\[
 \exp(z+w)
 = \sum_{n=0}^\infty \frac{(z+w)^k}{k!}
 = \sum_{n=0}^\infty
\]

\begin{align*}
\sum_{k=0}^N \frac{z^k}{k!} \cdot \sum_{j=0}^N \frac{w^j}{j!}
- \sum_{n=0}^N \frac{(z+w)^n}{n!}
&=
 \sum_{k\le n, j\le n}
\end{align*}
