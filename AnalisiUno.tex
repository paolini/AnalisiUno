...
chiamato \emph{logaritmo in base $a$}\marginpar{logaritmo},
...
che l'esponente conviene riscrivere la potenza con
...
\begin{enumerate}
\item se $a=0$ e $b=+\infty$ allora ${a_n}^{b_n} \to 0$;
\item se $a=0$ e $b=-\infty$ allora ${a_n}^{b_n} \to +\infty$;
\item se $a=+\infty$ e $b=+\infty$ allora ${a_n}^{b_n} \to +\infty$;
\item se $a=+\infty$ e $b=-\infty$ allora ${a_n}^{b_n} \to 0$;
\item se $a=+\infty$ e $b>0$ allora ${a_n}^b \to +\infty$.
\end{enumerate}
...
si ha
\[
  (a_n)^{b_n} = c^\enclose{\log_c\enclose{(a_n)^{b_n}}}
...
= a \cdot \enclose{\frac{n+1}{n}}^\alpha \to \frac 1 a \cdot 1^\alpha = \frac 1 a < 1
\]
...
Abbiamo quindi già definite la \myemph{somma} tra elementi di $\CC$ e la moltiplicazione
...
Data una successione $a_n$ di numeri reali possiamo considerare la successione
delle cosiddette \myemph{somme parziali}
\[
  S_n = \sum_{k=0}^{n} a_k.
\]
Scrivere più brevemente $S_n = \sum a_n$.
...
il \emph{carattere} della serie geometrica assume
...
\begin{theorem}[serie a termini positivi]
Se $a_n\ge 0$ la serie $\sum a_n$ non può essere indeterminata:
...
Siano $\sum a_n$ e $\sum b_n$ serie con
...
Se $a_n$ e $b_n$ sono successioni positive $a_n > 0$,
$b_n > 0$ e se
\[
   \frac{a_n}{b_n} \to 1
\]
...
%
\begin{proof}
Le serie a termini positive non possono essere indeterminate
...
\[
   a_n \le C b_n.
\]
Se la serie $\sum b_n$ converge anche $\sum C b_n$ converge e, per confronto,
...
Sia $\sum a_n$ una serie a termini positivi (cioè $a_n\ge 0$).
Se $\sqrt[n]{a_n} \to \ell $ con $\ell<1$ allora la serie converge.
\end{theorem}
%
\begin{proof}
...
Se $a_{n+1} / a_n \to \ell$ e $\ell <1$ allora la serie converge.
\end{theorem}
%
\begin{proof}
