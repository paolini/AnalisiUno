...
si dice anche che $a_n$ è \myemph{indeterminata}
...
\end{enumerate}
Determinare il \myemph{carattere} di una successione
...
Una successione $a_n$ si dice essere
\begin{enumerate}
\item \myemph{crescente}: se per ogni $n\in \NN$ si ha $a_{n+1} \ge a_n$;
\item \myemph{decrescente}: se per ogni $n\in \NN$ si ha $a_{n+1} \le a_n$;
\item \myemph{monotòna}: se crescente o decrescente;
\item \myemph{costante}: se crescente e decrescente;
...
\marginpar{continuità sequenziale}
...
\begin{theorem}[proprietà esponenziale]
\begin{enumerate}
\item $\exp_a(x)\in \RR$ (è finito);
\item se $a>1$ la funzione $\exp_a$ è strettamente crescente;
\item se $a>1$
\[
  \exp_a(x) = \inf\left\{\sqrt[q]{a^p}\colon p\in \ZZ, q \in \NN\setminus\{0\},
  \frac p q > x\right\};
\]
\item $\exp_a(p/q) = \sqrt[q]{a^p}$ se $p\in \ZZ$, $q\in \NN\setminus\{0\}$;
\item $\exp_a$ è sequenzialmente continua;
\item $\exp_a(x+y) = \exp_a(x) \cdot \exp_a(y)$ per ogni $x,y\in \RR$.
\end{enumerate}
\end{theorem}

\begin{proof}
Fissato $a>1$ consideriamo la funzione $f\colon \QQ \to \RR$ definita da
$f(p/q) =  \sqrt[q]{a^p}$.
...
\[
 ...
 = \sqrt[q]{a^p}
\]
dunque la definizione di $f(x)$ è ben posta perché dipende
solo da $x\in \QQ$
e non da come $x$ viene rappresentato tramite frazioni equivalenti.

Verifichiamo ora che $f$ è strettamente crescente. Se $x,y \in \QQ$
...
Inoltre per $x,y\in \QQ$ vale
\[
  f(x+y) = f(x)\cdot f(y)
\]
infatti
posto $x=p/q$, $y=n/m$ si ha
\begin{align*}
f(x+y)
&=a^{\frac{pm+nq}{qm}}
=\sqrt[qm]{a^{pm+nq}}
=\sqrt[qm]{a^{pm}\cdot a^{nq}}
=\sqrt[qm]{a^{pm}}\cdot \sqrt[qm]{a^{nq}}\\
&=\sqrt[q]{a^p}\cdot \sqrt[m]{a^n}
=f(x)\cdot f(y).
\end{align*}
Analogamente
\[
...
f(-p/q)
 ...
 = \frac{1}{f(p/q)}
\]
e quindi $f(x-y)=f(x)f(-y)=f(x)/f(y)$.
...
\end{align*}
Ovviamente $A_x < B_x$ (nel senso che per ogni $\alpha \in A$ e per ogni $\beta \in B$ si ha $\alpha < \beta$). Essendo $f$ strettamente crescente si
ottiene di conseguenza che $f(A_x) < f(B_x)$
e quindi $\sup f(A_x) \le \inf f(B_x)$.

Osserviamo che per definizione abbiamo posto
\[
 \exp_a(x) = \sup f(A_x)
\]
e che una delle proprietà che vogliamo dimostrare è
$\exp_a(x) = \inf f(B_x)$.

Vogliamo quindi mostrare che $\sup f(A_x) = \inf f(B_x)$.
Dato qualunque $x\in \RR$ esiste $k\in \NN$ tale che $x < k$.
Essendo $k\in B_x$ e $f(x) < f(k)$
otteniamo immediatamente che
\[
  \exp_a(x) = \sup f(A_x) \le \inf f(B_x) \le f(k) = a^k < +\infty.
\]
Inoltre scelto comunque $n \in \NN$, per la densità dei razionali
...
In particolare se $x\in \QQ$ e $y<x<z$ allora $f(y)< f(x) < f(z)$ e quindi
\[
 x \in \QQ \implies  \inf B_x = f(x) = \sup A_x
\]
che significa che $\exp_a(p/q) = \sqrt[q]{a^p}$.

Mostriamo ora che $\exp_a$ è strettamente crescente.
Presi $x,y\in \RR$ con $x<y$ esistono $z,w\in \QQ$ tale che $x<z<w<y$.
Allora $z\in B_x$ e $w\in A_y$, dunque
\[
  \exp_a(x) = \inf f(B_x) \le f(z) < f(w) \le \sup A_y = \exp_a(y).
\]

Per mostrare che $\exp_a$ è sequenzialmente continua dobbiamo
considerare una generica successione convergente $x_n \to x$
e mostrare che $\exp_a(x_n)\to \exp_a(x)$.
Dato $\eps>0$ essendo $\exp_a(x) = \sup f(A_x)$ deve esistere $t<x$
tale che $f(t) > \exp_a(x) -\eps$. Ma essendo $x_n\to x$ deve anche
esistere $N\in \NN$ tale che per ogni $n>N$ si ha $x_n>t$ e quindi
$f(x_n) > f(t) > \exp_a(x) -\eps$.
Analogamente essendo $\exp_a(x) = \inf f(B_x)$ deve esistere $s>x$
tale che $f(s) < \exp_a(x) + \eps$. Ma essendo $x_n\to x$ deve
esistere $M\in \NN$ tale che per ogni $n>M$ si ha $x_n<s$
e quindi $f(x_n) < f(s) < \exp_a(x) + \eps$. Risulta quindi
verificata la definizione di limite $\exp_a(x_n)\to \exp_a(x)$.

La relazione
\[
  \exp_a(x+y) = \exp_a(x) \cdot \exp_a(y)
\]
è verificata se $x,y \in \QQ$ (l'abbiamo già osservato).
...
Abbiamo fatto tutte le dimostrazioni nel caso $a>1$. Nel caso
$a<1$ i risultati si ottengono immediatamente usando la definizione
\[
  \exp_a(x) = \exp_{\frac 1 a}(-x)
\]
e riconducendosi quindi al caso $a>1$.
\end{proof}

\begin{comment}
\section{la funzione esponenziale}

In base al criterio del rapporto (o della radice) è facile osservare che
per ogni $z \in \CC$
la serie
\[
...
Definiamo la \myemph{funzione esponenziale} (complessa)
$\exp \colon \CC \to \CC$ come
\[
  \exp (z) = \sum_{k=0}^\infty \frac{z^k}{k!}.
\]
Definiamo il \myemph{numero di Nepero} (o di Eulero)
\[
  e = \exp (1).
\]
\end{definition}

\begin{theorem}[limite di Bernoulli]
Si ha
\[
  \lim_{n\to \infty} \enclose{1+\frac 1 n}^n = \sum_{k=0}^\infty \frac{1}{k!} = e.
\]
\end{theorem}
%
\begin{proof}
...
\[
 ...
 = \sum_{k=0}^n \frac{1}{k!} \frac{n!}{n^k\cdot (n-k)!}.
\]
...
\end{align*}
osserviamo che $c(n,k)\le 1$ in quanto prodotto di numeri minori o uguali ad $1$.
Inoltre, fissato $k$, si ha $c(n,k)  \stackrel n\to 1$ in quanto ogni fattore tende
...
\begin{comment}
Consideriamo la crescita di una popolazione (o di un capitale): sia $P(t)$ la popolazione al tempo $t$. La crescita nell'intervallo
temporale $[t_1,t_2]$ è $P(t_2)-P(t_1)$.
La velocità di crescita è $\frac{P(t_2)-P(t_1)}{t_2 - t_1}$ e il
tasso di crescita è la velocità di crescita rapportata
alla popolazione:
\[
  c = \frac{1}{P(t_1)}\cdot \frac{P(t_2) - P(t_1)}{t_2- t_1}.
\]

Se una popolazione cresce senza impedimenti (risorse illimitate)
è sensato considerare
che il tasso di crescita sia costante. Infatti la crescita sarà proporzionale al numero di individui che compongono alla popolazione,
in quanto ognuno contribuisce indipendentemente alla crescita.

Dividiamo l'intervallo temporale $[0,1]$ in $n$ intervalli ognuno
di ampiezza $1/n$ e denotiamo con $p_k$ la popolazione al $k$-esimo
istante. Allora si avrà
\[
  c = \frac{p_{k+1} - p_k}{p_k\enclose{\frac{k+1}{n} - \frac{k}{n}}}
\]
da cui
\[
  p_{k+1} = \enclose{1+\frac{c}{n}} p_k.
\]
Iterando si ottiene:
\[
 p_n = \enclose{1+\frac{c}{n}}^n p_0.
\]
Posto quindi
\[
  E_n(x) = \enclose{1+\frac{x}{n}}^n
\]
il limite di tale successione a $x$ fissato, se esiste,
rappresenterà
l'aumento percentuale effettivo in una unità di tempo per una
popolazione con tasso istantaneo di crescita pari a $x$.

\begin{theorem}[successione di Nepero]
La successione
\[
...
%
\begin{proof}
Dimostriamo innanzitutto, per induzione, che $a_n$ è crescente, cioè che
per ogni $n\ge 2$ si ha $a_n \ge a_{n-1}$.
E' chiaro che $a_n>0$,
quindi è sufficiente verificare che $\frac{a_n}{a_{n-1}} \ge 1$.
...
\]
da cui si ottiene, come volevamo, $a_n / a_{n-1} \ge 1$. Inoltre possiamo
osservare che
\[
