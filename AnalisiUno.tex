...
Nel seguito ometteremo l'avverbio "sequenzialmente" e parleremo
...
Dunque $\sup f^{-1}(A'y) = x = f^{-1}(y)$. In maniera analoga
...
ci garantisce che le condizione $f(x) = \sup A_x$ e $f(x)= \inf B_x$
garantiscono la continuità di $f$.

\begin{comment}
Per mostrare che $f(x)$ è continua dobbiamo
...
esistere $N\in \NN$ tale che per ogni $n>N$ si abbia $x_n>t$ e quindi
$f(x_n) > f(t) > f(x) -\eps$.
Analogamente essendo $f(x) = \inf f(B_x)$ deve esistere $s>x$
tale che $f(s) < f(x) + \eps$. Ma essendo $x_n\to x$ deve
esistere $M\in \NN$ tale che per ogni $n>M$ si abbia $x_n<s$
...
Se $a>1$:
\[
  x_n \to +\infty \implies a^{x_n} \to +\infty
\]
mentre
\[
  x_n \to -\infty \implies a^{x_n} \to 0.
\]
\end{theorem}
%
\begin{proof}
...
Il teorema sulla funzione esponenziale ci dice che $f(x)>0$ e quindi $f(\RR) \subset (0,+\infty)$. Visto però che $a^n\to +\infty$ e $a^{-n} \to 0$ scopriamo che $\sup f(\RR)=+\infty$ e $\inf f(\RR)=0$.
...
chiamato \myemph{logaritmo in base} $a$, la funzione inversa di $a^x$.
Si ha dunque
\[
  \log_a x = y \iff a^y = x.
\]
...
%
\begin{proof}
Dimostriamo innanzitutto, $a_n$ è crescente, cioè che
...
Data una successione $a_n$ possiamo considerare la successione
delle \myemph{somme parziali}
\[
  S_n = \sum_{k=0}^n a_k.
\]
Potremmo scrivere più brevemente $S_n = \sum a_n$.
\end{comment}
