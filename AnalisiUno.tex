...
\]
Abbiamo già osservato che $S_M$ converge: sia
\[
  E = \lim_{M\to +\infty} S_M
    = \sum_{k=0}^{+\infty} \frac{\abs{z}^k}{k!}.
\]

Consideriamo ora un qualunque $\eps \in (0,1]$, e scegliamo un $M$
(grazie alla definizione di limite $S_M \to E$) tale
che
\[
\abs{S_M - E} = \sum_{k=M+1}^{+\infty} \frac{\abs{z}^k} {k!} < \eps.
\]
Fissato $k\le M$ visto che $c(n,k)\to 1$
esiste $N_k > M$ tale che per ogni $n>N_k$
si ha $\abs{1-c(n,k)} < \eps$.
Prendiamo allora
\[
  N=\max_{k=0}^M N_k
\]
cosicchè per ogni $n>N$ e per ogni $k\le M$ si avrà $\abs{1-c(n,k)} < \eps$.
Allora, per ogni $n>N$
\begin{align*}
