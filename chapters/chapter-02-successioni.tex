...
Posto 
\[
B = \ENCLOSE{\frac{p}{2^n}\colon n\in \NN, p\in \NN, p\le 2^n}
\]
abbiamo definito $\phi\colon B\to U$ in modo che {eq:4012387}
sia soddisfatta. 
...
Osserviamo che sia ha $\phi(t) = \phi(s)\cdot \phi(t-s)$, 
...
Possiamo allora estendere la funzione $\phi$ per monotonia.
Fissato $t\in[0,1]$ consideriamo gli insiemi:
\[
  A_t = \ENCLOSE{\Re \phi(s)\colon s\in B, s\ge t}, \qquad
  B_t = \ENCLOSE{\Re \phi(s)\colon s\in B, s\le t}.
\]
Per quanto visto sopra sappiamo che $A_t \le B_t$ 
sono insiemi separati e dunque, 
per l'assioma di continuità,
ammettono elemento di separazione.
Vogliamo ora dimostrare che tale elemento di separazione è 
unico.

Supponiamo per assurdo che ci siano due elementi di separazione 
$t_1<t_2$ e poniamo $\eps=\frac{t_2-t_1}{2}$.
Innanzitutto vogliamo dimostrare che 
esiste $n\in \NN$ tale che $\abs{u_n-1}<\eps$.
Notiamo che
\[
\abs{z^2-1}=\abs{z+1}\abs{z-1}
\]
e se $z\in U\cap Q$ si ha $\abs{z+1}\ge \sqrt 2$ 
(il punto di $U\cap Q$ più vicino a $-1$ è $i$).
Dunque essendo $u_{n+1}^2=u_n$ si ha 
\[
 \abs{u_{n+1}-1} \le \frac{\abs{u_n}-1}{\sqrt 2}.
\]
Visto che $\abs{u_0-1}=\abs{i-1}=\sqrt 2$, 
per induzione si ottiene facilmente che 
$\abs{u_n-1}\le \frac{1}{\sqrt{2}^{n-1}}$.
Ricordando che $2^{n+1}\ge n+1$ (esercizio~\ref{ex:6734098})
deve esistere $n\in \NN$ per cui $2^{n-1} > \frac{1}{\eps^2}$
da cui $\abs{u_n-1} < \eps$.

Preso tale $n\in \NN$ scegliamo $p\in \NN$ in modo che 
$\frac{p}{2^n}\le t_1 \le \frac{p+1}{2^n}$.
Posto $z=\phi(p/2^n)$ e $u_n=\phi(1/2^n)$ si ha 
$\Re z \in B_t$ e $\Re (zu_n) \A_t$.
Se $z=a+ib$ e $u_n=x_n+iy_n$ si ha 
\[
  \Re \phi\enclose{\frac p {2^n}}-\Re \phi\enclose{\frac{p+1}{2^n}}
  = \Re z - \Re (zu)
  = a - (ax_n-by_n) 
  = a(1-x_n)+by_n 
  \le (1-x_n)+y_n
