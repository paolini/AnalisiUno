...
$a_n = n\cdot \Im \phi\enclose{\frac 1 n}$ converge 
ad un numero positivo. Possiamo quindi dare la seguente.
...
\[
 \pi 
  = \frac 1 2 \lim_{n\to +\infty} n\cdot \Im \phi \enclose{\frac 1 n}.
\]

Con questa scelta le funzioni trigonometriche $\sin$ e $\cos$ 
avranno, d'ora in poi, periodo $\tau = 2\pi$.
\end{definition}

La scelta di $\tau$ fatta nella definizione precedente 
è quella che garantisce che il punto di coordinate 
...
Per ogni $x\in\RR$, $x\neq 0$ si ha 
\mynote{ricordiamo che in questo teorema si suppone che le funzioni 
trigonometriche usino i radianti come unità di misura. 
In generale se la misura dell'angolo giro è $\tau$ 
tale limite vale $\frac{2\pi}{\tau}$.}
\[
...
\begin{figure}
  ...
  (mettendo in evidenza alcuni errori sulle ultime cifre decimali).
  ...
  (tutti i valori sono moltiplicati per $10^6$).
