...
\marginnote{In alcuni testi le successioni vengono indicate
...
\index{$e$}%
\footnote{John Napier (1550-1617) vedi note storiche a pag~\pageref{nota:Nepero}}%
\[
...
\index{Cesàro}%
 ...
 \]
 \end{exercise}
 \marginnote{Nell'esercizio\ref{ex:7340098} si dimostra che 
 \[
...
\footnote{vedi note storiche a pag~\pageref{note:isoperimetrico}}%
...
e viene chiamato \emph{curva}.
Impropriamente chiameremo \emph{curva} anche la funzione $\vec u$.
\marginnote{%
Invece che \emph{curva} sarebbe più appropriato 
il termine \emph{linea}. 
Visto che trattiamo una generica linea, anche non \emph{retta}, 
la chiamiamo \emph{linea curva} o più brevemente \emph{curva}.
}
...
Se $\ell(\vec u)<+\infty$ diremo che $\vec u$ descrive una curva 
\emph{rettificabile}.
\end{definition}

\begin{theorem}[proprietà geodetica]
Dati $\vec p, \vec q\in \RR^n$ la curva retta che congiunge 
$p$ a $q$ può essere descritta dalla funzione 
$\vec u(t) = (1-t) \vec p + t \vec q$, $\vec u\colon[0,1]\to \RR^n$.

Per tale curva (in realtà: retta) si ha 
\[
 \ell(\vec u) = \abs{\vec q- \vec p}.
\]
Inoltre tale curva è la più breve curva che congiunge $\vec p$ a $\vec q$ 
in quanto se $\vec v\colon[a,b]\to \RR^n$ è una qualunque curva con 
...
%
\begin{proof}
Per la curva \emph{retta} $\vec u$ si osserva che vale:
\[
  \abs{\vec u(t)-\vec u(s)} 
...
Viceversa se $\vec v$ è una qualunque curva che congiunge 
...
\begin{theorem}[additività della lunghezza]
  Siano $a\le b \le c$ e sia $\vec u\colon [a,c]\to \RR^n$ una curva.
  Allora%
  \marginnote{
...
\end{corollary}
\begin{proof}
Segue dall'additività, visto che la lunghezza di una curva 
...
Sia $\vec u \colon [c,d]\to \RR^n$ una curva.
Allora 
\[
...
\begin{lemma}[lunghezza curva lipschitziana]
...
\begin{lemma}[lunghezza di curve isometriche e omotetiche]
...
Dunque consideriamo la curva $\vec u\colon [-1,1]\to \RR^2$ definita da
\[
  \vec u(t) = (t,\sqrt{1-t^2}).
\]
Chiaramente i punti di questa curva si trovano sulla circonferenza trigonometrica $C$, 
più precisamente tale curva descrive la semicirconferenza superiore $C\cap\ENCLOSE{y\ge 0}$.
...
\label{th:ell_u}%
La curva $\vec u\colon [-1,1]\to \RR^2$, $\vec u(t) = (t,\sqrt{1-t^2})$
...
Ora osserviamo che la curva $\vec u\llcorner[0,1]$ 
...
lunghezza della curva $\vec u(t)$ con $t\in[x,1]$. 
Lo chiamiamo
\[
...
Se ora consideriamo la curva $\vec v = \vec u \circ \arccos^{-1}$
...
\[
 ...
 = \arccos ()\arccos^{-1} (t)) = t.
\]
Dunque la curva $\vec v\colon[0,\pi]\to\RR^2$ descrive la stessa
...
la circonferenza, tra tutte le curve piane di lunghezza prefissata, 
