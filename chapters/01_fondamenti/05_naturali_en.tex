...
understand the general sense of the discourse and then, if necessary, go back to
...
are isomorphic, meaning they can be put into a one-to-one correspondence while
...
We could simply require as an axiom that there exists a set $\NN$ that satisfies
...
a set $X$ is infinite if it can be put into a one-to-one correspondence 
...
and therefore cannot be put into correspondence with all of $X$.
...
\index{finite set}%
  ...
  if there exists $f\colon X\to X$ that is injective but not surjective.
...
%
\begin{proof}
If $X$ is infinite, there exists $f\colon X\to X$ that is injective but not
surjective.
...
the natural numbers, and therefore there are no \emph{unreachable} naturals
starting from zero.
...
But it is obvious that to every predicate $P$ corresponds 
...
We want to prove that $f$ is a function, i.e., that it is uniquely defined 
on all of $\NN$.
...
Now, prove that $f$ is unique. Consider 
...
But then $f\subset F$, and therefore $(\sigma(n),y)\not \in f$.
...
that $0$ is the identity element and that the associative 
and commutative properties hold.

Clearly, $n+0=n$ by definition~\eqref{eq:3408923}. 
Now verify that also $0+m=m$, and we do this by induction on $m$.
If $m=0$, it is true by definition.
For $0+\sigma(m)$, we have by definition:
$0+\sigma(m)=\sigma(0+m)$, and by the inductive hypothesis, $0+m=m$.
Thus $0+\sigma(m)=\sigma(m)$, which is what we wanted to prove.

Verify the associative property: $n+(m+k) = (n+m)+k$, and 
we do this by induction on $k$. 
If $k=0$, then $n+(m+0)=n+m=(n+m)+0$, by the previous properties.
For the inductive step, evaluate (always using definition~\eqref{eq:3408923}):
$n+(m+\sigma(k)) = n+\sigma(m+k) =\sigma(n+(m+k))$.
The inductive hypothesis guarantees that $n+(m+k)=(n+m)+k$, and thus:
$n+(m+\sigma(k)) = \sigma((n+m)+k) = (n+m)+\sigma(k)$,
which is what we wanted to prove.

Finally, verify the commutative property: $n+m=m+n$.
We do this by induction on $m$. 
If $m=0$, it is trivial due to the properties of the identity element.
The inductive step is immediately 
given by definition~\ref{eq:3408923} 
...
We must use the properties $n+\sigma(m) = \sigma(n+m)$ 
and $n+0=n$:
\begin{align*}
...
we can in the future avoid using the \emph{successor} function 
...
Define the ordering of $\NN$ 
with the idea that a number $m$ is \emph{greater} than $n$ 
...
and the ordering defined on $\NN$
...
identity element, absorbing element, associative property, 
distributive property, and all properties of powers.
That addition is commutative and associative has already been proven 
...
The property $n\cdot 1=n$ can be proved by induction.
...
The commutative property $n\cdot m=m\cdot n$ can be proved 
...
the product of two numbers different from $0$ could be zero.
...
To show that the ordering is total, we must proceed with an inductive proof. 
...
and prove that then $n+1\le m$ or $m\le n+1$.
...
Now prove the monotonicity of sum and product.
...
To verify the monotonicity of powers, proceed by induction.
...
this definition (controversial) will prove to be useful.
...
\subsection{finite sequences or tuples}
\index{tuples}%
\index{sequence!finite}%

We have already seen that if $A$ is a set, we can define the set 
of pairs of elements of $A$ with the Cartesian product of sets: 
$A\times A$. 
...
Using the Von Neumann definition, $3 = \ENCLOSE{0,1,2}$, the set 
...
$n\in\NN$,
we identify $A^{n}$ with the set of $n$-tuples (read: ennuples) of elements 
\index{tuples}%
of $A$:%
\mynote{
...
Moreover, the same definition will be usable 
...
of zero terms (or factors) equals the identity element 
of the operation, that is, $0$ for the sum (and $1$ for the product).
...
\begin{exercise}
  ...
  \]
\end{exercise}
The result of the sum can be remembered in the following way:
...
\end{equation}
On the right side appears the sum we want to calculate 
(multiplied by 3)
...
\mynote{%
it is called a \emph{telescoping sum}
\index{sum!telescoping}%
\index{telescoping}%
...
\mynote{This is also a \emph{telescoping sum}}
...
into correspondence so that zero maps to zero and corresponding numbers 
...
Obviously, also $b\in A$, and therefore it must be $b>a$ since
...
To efficiently represent any natural number, we use the \emph{decimal positional
notation}%
...
\]
Observing that $10 = 1\cdot d + 0 = d$, we write:
\begin{align*}
  4701 
...
of addition, where the sums of all pairs of decimal digits are reported
(table~\ref{tab:tabelline}).
Exploiting the definition and the distributive property of multiplication 
...
Once the \emph{tables} are memorized, it is possible to perform addition 
and multiplication operations on any pair of natural numbers expressed in
decimal form.
...
example.
Let us try to prove that $34\cdot 56 = 1904$:
\begin{align*}
\mathbf{34}\cdot \mathbf{56} 
  ...
  \mathbf{20})\cdot 10 \\
    &= \mathbf{1}\cdot 10^2+(\mathbf 8+\mathbf 2)\cdot 10 + \mathbf 4 + (\mathbf
  1\cdot 10^2 + (\mathbf 5+\mathbf 2)\cdot 10 + \mathbf 0)\cdot 10 \\
    &= (\mathbf 1+\mathbf 1)\cdot 10^2 +\mathbf 0\cdot 10 + \mathbf 4 + (\mathbf
  1\cdot 10^2 + \mathbf 7\cdot 10 + \mathbf 0)\cdot 10 \\
    &= \mathbf 2\cdot 10^2 + \mathbf 0\cdot 10 + \mathbf 4 + \mathbf 1\cdot
  10^3+\mathbf 7\cdot 10^2 + \mathbf 0 \cdot 10\\
  &= \mathbf 1\cdot 10^3 + \mathbf 9\cdot 10^2 + \mathbf 0\cdot 10 + \mathbf 4
  = \mathbf{1904}.
\end{align*}

\subsection{Factorial and Semi-Factorial}
...
as the product of all natural numbers from $1$ to $n$:
\[
  n! = \prod_{k=1}^n k.
\]
The recursive definition of the product allows us to define the factorial
as follows:
\[
  \begin{cases}
    0! = 1 \\
...
\begin{table}
  ...
  \footnotesize $10^{3n}$  &  & K (kilo) & M (mega) & G (giga) & T (tera) \\ 
  \footnotesize $10^{3(n+5)}$  & P (peta) & E (exa) & Z (zetta) & Y (yotta) \\ \hline
  \footnotesize $2^{10n}$  &  & Ki (kibi) & Mi (mebi) & Gi (gibi) & Ti (tebi) \\
  \footnotesize $2^{10(n+5)}$ & Pi (pebi)& Ei (exbi) & Zi (zebi) & Yi (yobi)
  \end{tabular}
  \end{center}
  \caption{The first values (and names) of some of the sequences we have defined.
  It will be particularly useful to remember that $2^{10}=1024$ 
  is very close to $10^3=1000$: this justifies the similar names used
  for the powers of $2^{10}$ and for the powers of $10^3$.
  }
  \end{table}
  
  Sometimes it will be useful to consider the products of only the even numbers
  or the odd numbers up to a certain number $n$. This
  is called the \emph{semi-factorial}%
\mymargin{semi-factorial}%
\index{semi-factorial} and is denoted by $n!!$
  We can define it separately for even numbers (i.e., 
  numbers that can be written in the form $2n$ with $n\in \NN$)
  and odd numbers (which we write in the form $2n+1$):
  \index{"!"!}
  \index{$n$"!"!}
  \index{double factorial}%
  \index{factorial!double}%
  \index{factorial!semi-factorial}%
  \index{semi-factorial}%
  \begin{align*}
  ...
  \]
  These formulas allow the semi-factorial to be expressed using
  the full factorial and powers.
  \end{remark}
  
\begin{exercise}
  Provide an inductive definition of the semi-factorial
  (separately for even and odd numbers)
  and prove, by induction, the formulas in the previous remark.
\end{exercise}

\subsection{Finite Cardinalities}

We denote by $[n] = \ENCLOSE{k\in \NN\colon k<n}$
the set of the first $n$ natural numbers.
We will have $[n]=\ENCLOSE{0,1,\dots, n-1}$.
\mynote{
  Note that if $\NN$ is the set 
  of Von Neumann ordinals \eqref{eq:vonNeumann} then $[n]=n$.
}

\begin{definition}[finite cardinalities]
If $A$ is a set and $n\in \NN$, we say that 
$A$ has $n$ elements (or has cardinality $n$) and we write 
\mymargin{number of elements}%
\[
  \#A = n
\]
if there exists a bijective function $f\colon [n]\to A$.
\end{definition}

Intuitively, if $f\colon [n]\to A$ is any function, 
setting $a_1=f(0)$, $a_2=f(1)$, \dots, $a_{n}=f(n-1)$ 
we will have that the image of $f$ is $f([n]) = \ENCLOSE{a_1,a_2,\dots,a_n}$.
If $f$ is surjective, $A=f([n])$, and if $f$ is injective, 
all the elements $a_1$, \dots, $a_n$ will be distinct,
and thus indeed the set $A=\ENCLOSE{a_1,a_2,\dots,a_n}$
has $n$ elements.

\begin{theorem}[finite cardinalities]
  \label{th:cardinali_finiti}%
A set $A$ is finite
(in the sense of Dedekind, Definition~\ref{def:infinito})
if and only if there exists $n\in \NN$ such that $\#A = n$.
Moreover, such $n$, if it exists, is unique.
\end{theorem}
\begin{proof}
\emph{Step 1.} First, we prove that $[n]$ is finite.
This involves proving that if $f\colon [n]\to [n]$ is injective, then it is also
surjective.
We do this by induction. 
For $n=0$, note that $[0]=\emptyset$ and there is a unique function $f\colon
\emptyset \to \emptyset$
(the empty function: $\emptyset^\emptyset = \ENCLOSE{\emptyset}$)
which is indeed bijective.
Now suppose that $[n]$ is finite and that $f\colon [n+1]\to [n+1]$ is injective.
We can then define $g\colon[n]\to[n]$ by setting:
\[
  g(k) = \begin{cases}
    f(k) & \text{if $f(k)\neq n$,}\\
    f(n) & \text{if $f(k)=n$.}
  \end{cases}
\]
With a little care, it is easily verified that if $f$ is injective, then $g$ is
also injective
(the idea is that $g$ uses the same values as $f$ discarding the value $n$,
since $n\not \in [n]$,
and using instead the value $f(n)$).
But then, by the inductive hypothesis, $g$ is bijective.
Thus, $f$ is also bijective because it assumes all the values assumed by $g$  
and, having one more point and being injective, it must assume an additional
value which can only be $n$.

\emph{Step 2.}
We prove that if $\#A=n$, then $A$ is finite.
Given $f\colon A\to A$ injective, we must prove that $f$ is surjective.
By hypothesis, $\#A=n$, thus there exists a bijective function $g\colon A\to
[n]$.
The function $h=g\circ f\circ g^{-1}\colon [n]\to [n]$ is injective, as it is a
composition of injective functions.
Thus, since $[n]$ is finite by Step 1, the function $h$ is also surjective.
But then $f$ is also surjective because $f=g^{-1}\circ h\circ g$.

\emph{Step 3.} We prove that if $f\colon[n]\to A$ is injective
and there exists $g\colon[n+1]\to A$ injective, then $f$ cannot 
be surjective.
Indeed, if $f$ were bijective, then the function
$h\colon [n+1]\to [n]$ defined by $h=f^{-1}\circ g$ 
would be injective.
But its restriction $h'=h \llcorner [n]$ would be a function
injective $h'\colon[n]\to[n]$ that would never assume the value 
$h(n)$, thus it would not be surjective.
By definition, this means that $[n]$ is infinite, 
which we have already excluded in Step 1.

\emph{Step 4.}
We prove that if $A$ is finite, then there exists $n\in \NN$ such that $\#A=n$.
We proceed by contraposition and assume that there does not exist $n\in \NN$
such that $\#A=n$:
we must prove that in this case $A$ is infinite.
To do this, consider the families of functions:
\[
  J_n(A) = \ENCLOSE{f\colon [n]\to A\colon f \text{ injective}}.
\]

Obviously, if $f\in J_{n+1}(A)$, then $f$ restricted to $[n]$ is an element of
$J_n(A)$.
Thus, if $J_n(A)=\emptyset$, then also $J_{n+1}(A)=\emptyset$.
 
If $J_n(A)\neq \emptyset$ and $J_{n+1}(A)=\emptyset$, then we can assert that
$\#A=n$
because given an injective function $f\in J_n(A)$, if it were not surjective, we
could
extend it to an injective function $f'\in J_{n+1}(A)$.

Thus, it must be that $J_n(A)\neq \emptyset$ for every $n\in \NN$.

By Step 3, we know that every 
$f\in J_n(A)$ is not surjective, thus it can be extended 
to a function $g\in J_{n+1}(A)$
such that $g$ coincides with $f$ on $[n]$. 
\mynote{here the axiom of choice is used}
There must therefore exist a sequence of functions
$f_n\colon [n]\to A$ such that $f_{n+1}$ coincides with $f_n$ on $[n]$.
The graphs of these functions are thus an increasing sequence of sets, and their
union is the graph of a function $f\colon \NN \to A$ which also 
coincides with $f_n$ on $[n]$. 
Thus, $f$ is injective because if it were $f(n)=f(m)$
with $m<n$, it would also be $f_n(n)=f_n(m)$, which cannot be since each $f_n$
is injective.
On the image of $f$, I have a structure isomorphic to $\NN$, which I can exploit
to
construct an injective but not surjective function on $A$.
Define $h\colon A\to A$ by setting:
\[
  h(x) = \begin{cases}
    f(f^{-1}(x)+1) &\text{if $x$ is in the image of $f$},\\
    x &\text{otherwise}
  \end{cases}
\]
obviously, $h$ is injective (since $f$ is) 
but not surjective (since $f(0)$ is not in the image of $h$).

We have thus shown that $A$ is infinite, as desired.

\emph{Step 5.}
We prove that if $A\subset B$, $\#A=m$, $\#B=n$, then $m\le n$.
If, for the sake of contradiction, it were $\#A=n$ and $\#A=m$ with $m>n$, then 
we would have a contradiction with what was stated in Step 3
since there should exist a bijective function $f\colon[n]\to A$ 
(since $\#A=n$) and there should exist an injective function 
$g\colon[n+1]\to A$ since, being $\#A=m$, there is an injective function 
$[m]\to A$ that I can restrict to $[n+1]$.
\end{proof}

\begin{theorem}[operations with finite cardinalities]
  \label{th:combinatoria}
  If $A$ and $B$ are finite sets, it holds that:
  \begin{align}
    \#(A\cup B) &= \#A + \#B \quad \text{if $A\cap B=\emptyset$},\\
    \#(A\setminus B) &= \#A - \#B \quad \text{if $B\subset A$},\\
    \#(A\times B) &= \#A \cdot \#B, \\
    \#(B^A) &= \#B^{\#A}, \\
    \#(A!) &= (\#A)!, \\
    \#(\P(A)) &= 2^{\#A}, \\
    \#\ENCLOSE{A\subset B \colon \#A=k} &= \binom{\#B}{k}.
  \end{align}
\end{theorem}
\begin{proof}
Regarding the disjoint union, if $f\colon [n]\to A$ and $g\colon [m]\to B$
are bijective, a bijective function $h\colon [n+m] \to A\cup B$ can be
defined as follows:
\[
  h(k) = \begin{cases}
    f(k) & \text{if } k<n,\\
  ...
  \end{cases}
\] 
It is easily verified that $h$ is bijective.
Regarding the difference $A\setminus B$, note that if $B\subset A$,
then $A = (A\setminus B) \cup B$, and thus $\#A = \#(A\setminus B) + \#B$ by the
previous point.

In the remaining cases, without loss of generality, 
we can assume that $A=[n]$. We proceed by induction on $n$.

For the Cartesian product, the case $n=0$ corresponds to $\emptyset\times B =
\emptyset$.
The inductive step is proven by observing that $[n+1]\times B = [n]\times B \cup
\ENCLOSE{n} \times B$,
from which, passing to cardinalities, it becomes $(n+1)\cdot m = n\cdot m + m$.

The elements of $B^{[n+1]}$ are the functions $[n+1]\to B$, 
and they can be uniquely identified
by taking a function $f\colon [n]\to B$ and a value $b\in B$ to assign 
to the point $n$. 
Thus, $B^{[n+1]}$ is in bijective correspondence with $B^{[n]}\times B$,
and passing to cardinalities, we obtain $m^{n+1} = m^n\cdot m$.

The set $A!$ is the set of bijective functions $A\to A$. 
Since $A$ is finite, the set of bijective functions coincides 
with the set of injective functions. 
Assume $A=[n]$ and proceed by induction on $n$.
If $n=0$, we have $\emptyset!=\ENCLOSE{\emptyset}$, which, passing to
cardinalities, becomes $0!=1$.
For the inductive step, observe that an injective function $[n+1]\to B$ 
is uniquely identified by choosing the value $b\in B$ to assign to the point $n$
and then taking a function $f\colon [n]\to B\setminus\ENCLOSE{b}$ injective. 
Thus, $[n+1]!$ is identified with $B\times [n]!$, and passing to cardinalities,
this becomes
the recursive definition of the factorial: $(n+1)! = (n+1)\cdot n!$.

For the power set, observe that $\P(A)$ is in bijective correspondence with
the set of functions $A\to \ENCLOSE{0,1}$: just associate with each subset $X$
of $A$
the function that is $1$ on $X$ and $0$ otherwise. 
Thus, $\#(\P(A)) = \#([2]^A) = 2^{\#A}$.

In the last point, we need to count the subsets of $B$ with $k$ elements.
Setting $n=\#B$, we can proceed by induction on $n$.
If $n=0$, then $B=\emptyset$, and the only subset 
is the empty set, which has $0$ elements: thus
the result is true because indeed $\binom{0}{k} = 0$ if $k\neq 0$ and
$\binom{0}{0}=1$.
The number of subsets of $B$ with $k+1$ elements 
can be determined as follows.
Fix an element $b\in B$ and consider the set 
$B'=B\setminus \ENCLOSE{b}$. Obviously, $\#B' = n-1$.
A subset of $B$ with $k+1$ elements can contain $b$ 
or not. 
The number of subsets of $B$ with $k+1$ elements that contain $b$
is equal to the number of subsets of $B'$ with $k$ elements,
while the number of subsets of $B$ with $k+1$ elements that do not contain $b$
is equal to the number of subsets of $B'$ with $k+1$ elements.
Thus, by the inductive hypothesis, the number of subsets of $B$ with $k+1$
elements is
\[
  \binom{n-1}{k} + \binom{n-1}{k+1} = \binom{n}{k+1}
\]
where the last equality is the recurrence relation for binomial coefficients
(see Theorem\ref{th:tartaglia})
\end{proof}

\begin{theorem}[pigeonhole principle]
  \index{principle!pigeonhole}%
  \index{pigeonhole principle}%
  \index{pigeonhole principle}%
If $A$ and $B$ are finite sets and $\#A > n\cdot \#B$, then 
for every $f\colon A \to B$, there exists $b\in B$ such that 
\[
 \# f^{-1}(\ENCLOSE{b}) \ge n+1.
\]
\end{theorem}
\begin{proof}
We prove the contrapositive implication: if for every $b\in B$, it holds that
$\# f^{-1}(b) < n+1$,
then $\#A \le n\cdot \#B$.
Without loss of generality, assume $A=[m]$.
Given the function $f\colon A\to B$, we can define a new function $g\colon A\to
B\times[n]$
by setting:
\[
  g(a) = \left(f(a),\# \ENCLOSE{x\in [a]\colon f(x)=f(a)}\right).
\]
We have essentially attached a different label to each value of $f$. 
In this way, it is easy to convince oneself that $g$ is injective.
Thus, $\#A \le \#(B\times [n]) = n\cdot \# B$, which is the contrapositive
thesis.
\end{proof}

\begin{example}
In a group of 1000 people, there are at least three people who have their
birthday on the same day.
The pigeonhole principle is applied with $A$ as the set of people, 
$B$ as the set of days of the year, and $f\colon A\to B$ associating each person
with the day
of their birthday. Since $1000 > 366 \cdot 2$, the theorem applies with $n=2$.
\end{example}