...
Nel capitolo~\ref{ch:edo} ci sarà molto utile valutare un polinomio su un operatore 
...
Queste verranno chiamate \myemph{espressioni polinomiali} su $\KK$
...
nell'espressione può comunque essere inserita mettendo
a zero il coefficiente corrispondente.
...
Se $\KK$ è un campo denoteremo con 
$\KK[x]$ \index{$\KK[x]$}%
l'insieme
di tutti i polinomi con coefficienti in $\KK$ e variabile $x$. 
Formalmente $\KK[x]$ è il quoziente dell'insieme 
di tutte le espressioni polinomiali rispetto alla relazione di equivalenza,
per cui si considerano uguali in $\KK[x]$ le espressioni con la stessa forma canonica.
Ogni polinomio di grado $n$ è univocamente
determinato dalla lista dei suoi coefficienti 
\[
  (a_0, \dots, a_n) \in \KK^{n+1}
\]
(una lista vuota nel caso $n=-\infty$).
I polinomi possono essere sommati tra di loro
e moltiplicati per qualunque elemento di $\KK$
...
Più esplicitamente se $P$ e $Q$ sono polinomi rappresentati da
\[
