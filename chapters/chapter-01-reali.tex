...
Visto che $\RR$ è, a meno di isomorfismi, l'unico campo 
ordinato e continuo, 
potremmo semplicemente supporre che $\RR$ sia un qualunque 
campo ordinato continuo. 
Le sue proprietà sono infatti caratterizzanti.
Questo è quello che viene fatto in molti libri di testo come ad esempio
\cite{Giusti} e \cite{Marcellini}.
A questo punto si può anche prendere per assioma l'esistenza di $\RR$, invece che 
l'esistenza di $\NN$, in quanto i due fatti sono equivalenti. 
Nei capitoli precedenti abbiamo mostrato che partendo da $\NN$ si può costruire $\RR$ 
(passando da $\ZZ$ e $\QQ$). 

Viceversa è facile identificare, dentro $\RR$, un sottoinsieme $\NN$ che soddisfi gli 
assiomi di Peano. Si considera in $\RR$ la proprietà induttiva: $X\subset \RR$ si dice 
essere \emph{induttivo} se $0\in X$ e se $n\in X$ implica $n+1\in X$. 
Si definisce quindi $\NN$ come il più piccolo sottoinsieme induttivo di $\RR$:
\[
