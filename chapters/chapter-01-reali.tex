...
\begin{example}
Se prendiamo l'insieme $X=\ENCLOSE{1,2,3}$ e consideriamo la relazione
su $\mathcal P(X)$ definita da $A\le B$ se $A\subset B$ otteniamo una relazione 
d'ordine che non è totale. 
Infatti se $A=\ENCLOSE{1,2}$ e $B=\ENCLOSE{2,3}$ non è vero né che $A\subset B$ 
né che $B\subset A$.
\end{example}
...
\mynote{
  ...
  Viceversa se $A$ è infinito il massimo e il minimo 
