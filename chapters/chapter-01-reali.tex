...
Le regole di inferenza che riguardano il calcolo delle proposizioni
...
due regole formali: una che permette di introduzione l'operatore e una 
che permette di eliminazione l'operatore.
...
Ad esempio se abbiamo i teoremi $1+1=2$ e $2+1=3$ 
...
che anche $1+1=2 \lor \not 1+1=2$ è un teorema.
La regola di eliminazione della disgiunzione è leggermente 
più complicata. Se sappiamo che $P\lor Q$ e se entrambi 
$P\implies R$ e $Q\implies R$ sono teoremi, possiamo 
dedurre che $R$ è un teorema.
Ad esempio se $x>1 \implies x^2>1$ 
e $x<-1 \implies x^2>1$ sono teoremi e se anche 
$x>1 \lor x<-1$ è un teorema possiamo allora dedurre che $x^2>1$ 
è un teorema.
...
La regola ci dice che se abbiamo i teoremi 
$P$ e $P\implies Q$ possiamo dedurre che anche $Q$ è un 
teorema.
...
Infine abbiamo le regole di introduzione ed eliminazione 
...
e cioè che $F\implies V$ (dove $F$ sta per falso e 
$V$ sta per vero). 
...
Si noti che $P$ e $Q$ non sono simboli del sistema formale 
ma rappresentano proposizioni qualunque. 
Si tratta di simboli segnaposto. 
Ad esempio ponendo $P$ è $1\neq 1$ e $Q$ è $1=1$ 
vorremo dire che $1\neq 1 \implies 1=1$ è un teorema.
Proviamo a fare una dimostrazione formale.
...
Ovviamente sono formule ben formate tutte le proposizioni 
che si ottengono mediante l'utilizzo degli operatori 
logici e delle parentesi. Inoltre supponiamo siano 
valide le regole di inferenza della deduzione naturale
che abbiamo esposto poco sopra.
