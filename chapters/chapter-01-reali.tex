...
L'ultimo assioma dei numeri naturali 
...
\index{denso} o \emph{divisibile} 
  ...
  \mymargin{Più in generale se $R$ è un insieme totalmente 
...
Si osservi che su $\RR_+$ la definizione di potenza $x^n$ 
...
Così come su $\RR$ il teorema~\ref{th:divisibile}
ci garantiva l'esistenza della frazione $\frac x n$ così 
lo stesso teorema, applicato al gruppo moltiplicativo $\RR_+$,
ci garantisce l'esistenza della radice $n$-esima: $\sqrt[n]{x}$.
...
\begin{exercise}
Si provi a dimostrare \ref{eq:approx_sqrt2} senza 
...
(se non teniamo conto del fatto che l'esponente $d$ è limitato, tipicamente )
\mymargin{In un moderno calcolatore a 64 bit la rappresentazione binaria (float64) 
...
Definiamo $a^x = \phi_a(x)$ e lo chiamiamo \emph{funzione esponenziale}
