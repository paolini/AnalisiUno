...
\index{denso} o \emph{divisibile} 
  ...
  se dati $x,y\in R$ con $x<y$ esiste $c\in A$ tale che 
  $x<c<y$.}
  \[
...
Definiamo la somma di due angoli orientati definendo 
...
\mynote{Si noti che $A\cap B=\emptyset$, $A\cup B=U$, 
$1\in A$ e $-1\in B$.}
\[
...
Dunque definiamo
\mynote{
Quando sommiamo due angoli orientati $(k_1,u_1)$ e $(k_2,u_2)$ 
dobbiamo sommare tra loro i giri completi $k_1+k_2$ e moltiplicare tra loro 
i numeri complessi rappresentanti gli angoli $u_1\cdot u_2$. 
Poi però dobbiamo aggiungere un giro in più se il prodotto $u_1\cdot u_2$ 
``sfora'' l'angolo giro. 
Questo succede certamente se entrambi gli angoli si trovano in $B$ 
(sono entrambi maggiori di 180 gradi) oppure se almeno uno dei due 
si trova in $B$ e la somma geometrica si trova in $A$ (se uno dei due 
è maggiore di 180 gradi e l'altro minore, la somma potrà essere 
tra i 180 e i 540 gradi).
}%
...
\end{equation}
\mynote{
Ad esempio $(0,i) \stackrel G+ (0,-i) = (1,1)$ (in gradi sarebbe 
$90+270 = 360$),
$(1,-i) \stackrel G+ (-1,-1) = (1,i)$ (in gradi $630-180=450$).}
...
$u_1$ & $u_2$ & $u_3$ & $u_{12}$ & $u_{23}$ &$u_{123}$ & $k$\\\hline
A & A & A & A & A & A & 0\\
A & A & A & A & A & B & 0\\
A & A & A & A & B & A & -\\
A & A & A & A & B & B & 0\\
...
Chiaramente $(0,1)$ è elemento neutro: 
...
\mynote{$n\cdot g$ è la somma ripetuta: $g+\dots +g$.}%
\mynote{Ogni sottoinsieme non vuoto e inferiormente limitato 
di $\ZZ$ ha minimo perché ci si può ricondurre 
al buon ordinamento di $\NN$.}%
...
\end{proof}
%
\begin{proof}[Dimostrazione teorema~\ref{th:gruppo_su_U}]
...
\]
\mynote{Quando avremo definito l'esponenziale complesso 
scopriremo che $\phi(x) = e^{2i\pi x}$.}%
con le seguenti proprietà:
\begin{enumerate}
  \item $\phi(x+y) = \phi(x)\cdot \phi(y)$ per ogni $x,y\in\RR$ (proprietà di omomorfismo);
  \item $\phi(0) = \phi(1) = 1$;
  \item $\Im \phi\colon \closeinterval{0}{1/4}\to[0,1]$ 
  è strettamente crescente e bigettiva;
  \item $\Re \phi\colon \closeinterval{0}{1/4}\to[0,1]$ 
...
\begin{proof}
  ...
  Se restringiamo la funzione $f$ all'intervallo $\closeinterval{0}{1/4}$
  otteniamo una funzione bigettiva tra tale intervallo 
  e l'intervallo in $G$ dei punti compresi tra $f(0)=(0,1)$ e $f(1/4)=(0,i)$.
  ...
  crescente e di conseguenza $\Im \phi \colon \closeinterval{0}{1/4}\to [0,1]$ 
  è iniettiva. 
  Ma tale funzione è anche suriettiva in quanto preso 
  qualunque $y\in [0,1]$ posto $x=\sqrt{1-y^2}$ si 
  trova che il punto $(0,x+iy)\in G$ è un punto 
  di $G$ compreso tra $(0,1)$ e $(0,i)$.
  Essendo $f$ bigettiva si trova dunque $t\in[0,1/4]$ 
  tale che $f(t) = (0,x+iy)$ da cui $\phi(t)=x+iy$ e 
  $\Im \phi(t) = y$.
  Sempre sull'intervallo $\closeinterval{0}{1/4}$ si ha 
  $\Re \phi(x) = \sqrt{1-\Im^2 \phi(x)}$ 
  in quanto $\abs{\phi(x)} = 1$ e $\Re \phi(x)\ge 0$,
  dunque $\Re \phi$ è strettamente decrescente e quindi 
  iniettiva.
  Anche $\Re \phi\colon \closeinterval{0}{1/4}\to [0,1]$
  è suriettiva in quanto scelto $x\in[0,1]$ e posto 
  $y=\sqrt{1-x^2}$ si trova $t$ come sopra in modo che 
  $f(t)=(0,x+iy)$ da cui $\Re \phi(t) = x$.
...
\begin{theorem}[esistenza funzioni trigonometriche]%
\label{def:sin_cos}%
\label{th:proprieta_trigonometriche}%
Per ogni $\tau>0$ esistono due funzioni $\sin,\cos\colon \RR\to \RR$ 
tali che per ogni $x,y\in \RR$ valgono le seguenti proprietà:
\mynote{Quando avremo definito $\pi$ definiremo 
...
Visto che $\Im \phi\colon[0,1/4]\to[0,1]$ è strettamente crescente e bigettiva
deduciamo che anche $\sin\colon[0,\tau/4]\to [0,1]$ lo è.
Ma $\sin(-x) = -\sin x$ e dunque è facile convincersi che 
anche $\sin \colon [-\tau /4,\tau/4]$ è strettamente 
crescente e bigettiva.
Analogamente dalle proprietà di $\phi$ si deduce che 
$\cos\colon [0,\tau / 4] \to [0,1]$ è strettamente 
...
la funzione $\cos x$ è strettamente decrescente e assume 
tutti i valori dell'intervallo $[-1,0]$.
\end{proof}
