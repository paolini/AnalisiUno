...
Se $G$ è un gruppo totalmente ordinato con elemento neutro $e$ si ha:
\[
  x\le y  \iff e\le x * y^{-1}.
\]
...
\begin{example}
Vedremo che $\QQ$ è un esempio di campo ordinato denso ma non continuo 
mentre $\RR$ è l'unico esempio di campo ordinato continuo.
Vedremo anzi che $\RR$ è anche l'unico gruppo additivo totalmente ordinato denso, continuo non banale.
\end{example}
...
Queste definizioni catturano l'idea che la somma è una ripetizione dell'operazione di successore:
$n+k$ si ottiene applicando $k$ volte la funzione $\sigma$ ad $n$,
la moltiplicazione è una somma ripetuta: 
$n\cdot k = \underbrace{n+\dots+n}_{k \text{ addendi}}$,
e l'elevamento a potenza è un prodotto ripetuto: 
$n^k = \underbrace{n\cdot \ldots \cdot n}_{k \text{ fattori}}$.
...
\begin{proof}
  ...
  I casi base sono sempre banali, verifichiamo sinteticamente i passi induttivi.
  
  Cominciamo con le proprietà della addizione.
  Ricordiamo che la definizione induttiva 
  garantisce che $n+(m+1) = (n+m)+1$.
  Il passo induttivo per dimostrare 
  la proprietà associativa è 
  \[
  (n+m)+(k+1) 
  = ((n+m)+k)+1 
  = (n+(m+k))+1
...
Passiamo alle proprietà delle potenze.
...
\begin{exercise}
Si consideri la funzione definita per ricorrenza da 
\[
\begin{cases}
  f(0) = 0,\\
  f(n+1) = f(n) + (n+1)^3.
\end{cases}
\]
Dimostrare che $f(n) = \frac{n^2(n+1)^2}{4}$
...
come segue. 
Se $A,B$ sono sottoinsiemi di $\QQ$ diremo che la coppia $(A,B)$
è una sezione di Dedekind se $A$ è l'insieme dei minoranti di $B$ 
e $B$ è l'insieme dei maggioranti di $A$.
Questo significa che se $b\in B$ se e solo se $b\ge a$ per ogni $a\in A$
e $a\in A$ se e solo se $a\le b$ per ogni $b\in B$.
Osserviamo che necessariamente $A\cup B=\QQ$ infatti preso qualunque 
$x\in \QQ$ se $x$ è un maggiorante di $A$ si ha $x\in B$, se invece 
$x$ non è un maggiorante di $A$ deve esistere $a\in A$ tale che $x\le a$.
Ma visto che $a$ è un minorante di $B$ e $x\le a$ anche $x$ è un 
minorante di $B$ e quindi $x\in A$.

Per ogni $q\in \QQ$ possiamo prendere $A=\ENCLOSE{x\in \QQ\colon x\le q}$
e $B=\ENCLOSE{x\in \QQ\colon x\ge q}$ e 
risulta facilmente che la coppia $(A,B)$ è una sezione di Dedekind.


Definiamo quindi l'insieme dei numeri reali come l'insieme 
delle sezioni di Dedekind:
\index{reali!insieme dei}%
\index{numero!reale}%
\index{insieme!numeri reali}%
\[
\RR = \ENCLOSE{(A,B) \in \mathcal P(\QQ)\times \mathcal P(\QQ)\colon 
A,B\neq \emptyset, \text{ $(A,B)$ sezione di Dedekind}}/\sim
\]
...
\mynote{Ovviamente possiamo utilizzare le approssimazioni per 
eccesso invece che quelle per difetto. 
Un approccio leggermente diverso è quello di prendere le cosiddette 
\emph{sezioni di Dedekind} ovvero le coppie $(A,B)$ di sottoinsiemi 
non vuoti di $\QQ$ tali che $A\le B$ 
(nel senso che $a\le b$ per ogni $a\in A$ e ogni $b\in B$)
e tali che $A\cup B=\QQ$. 
Purtroppo anche in questo caso bisogna mettere una relazione di equivalenza 
perché se i due insiemi $A$ e $B$ hanno un elemento di separazione in 
$\QQ$ allora tale elemento può essere messo indifferentemente in $A$, in $B$
o in entrambi.
}
...
di Peano (definizione~\ref{def:assiomi_peano}) sono soddisfatte.
