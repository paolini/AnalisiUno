...
Nel seguente teorema vogliamo definire la moltiplicazione
come somma ripetuta e l'elevamento a potenza come moltiplicazione ripetuta.

\begin{theorem}[operazioni iterate]
Se $M$ è un monoide additivo con zero (definizione~\ref{def:monoide})
...
Lo stesso ovviamente si può fare nei monoidi moltiplicativi con unità
per definire la potenza $x^n$ come prodotto ripetuto:
\[
  x^0 = 1 \in M \\
  x^{n+1} = x^n \cdot x.
\]
...
al posto della somma su $M$ scriviamo un prodotto, al posto del prodotto 
scriviamo una potenza. Le operazioni su $\NN$ invece rimangono le stesse.
\end{proof}
...
\section{unicità dei numeri reali}
