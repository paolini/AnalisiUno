...
\begin{theorem}[sign permanence]%
  \label{th:permanenza_segno}%
\mymark{***}%
\index{sign permanence}%
\index{theorem!of sign permanence}%
\mymargin{sign permanence}%
If
\[
  \lim_{x\to x_0} f(x) > 0
\]
...
such that $V\subset (0,+\infty]$: if $\ell\in \RR$, it suffices to take 
$V=(\ell/2,3 \ell/2)$, if $\ell=+\infty$, it suffices to take $V=(1,+\infty]$.
By the definition of limit, there exists a neighborhood $U$ of $x_0$ 
such that $f(U)\subset V$ and the result follows.

For the converse, we argue by contradiction. 
If the limit $\ell$ of $f$ existed and were $\ell < 0$,
then by the previous point (applied to $-f$), we would 
conclude that there is a neighborhood of $x_0$ in which $f(x)<0$.
But then it is not possible for there to be a neighborhood in which $f(x)\ge 0$.
\end{proof}

\begin{theorem}[operations with function limits]
...
The cases in which the operations are not defined are usually called 
...
\]
(informally, we could write: $\frac{1}{0^+} = +\infty$).
...
and first the case in which $\ell_1$ and $\ell_2$ 
are both finite. 
In this case,
...
and this is even more true if we take $V=V_1\cap V_2$ 
instead of $V_1$ and $V_2$. 
Since $U_1+U_2 = U$, we will then have 
...
For the difference, it suffices to observe that $f(x)-g(x) = f(x) + (-g(x))$.
...
Regarding the product, we recall that thanks 
...
Changing the sign of one or both functions, we can 
reduce to the case $f > 0$ and $g > 0$ when the two functions assume 
a constant sign in at least one neighborhood of the point $x_0$.
By the Theorem of sign permanence, this is true if 
...
is equivalent to $\abs{f(x)}\to 0$, thus in this case, we can replace $f(x)$
with $\abs{f(x)}\ge 0$
...
multiplicative properties of the reciprocal, at least if we are in the realm of
positive numbers.
Thus, if $f(x)\to \ell$ with $\ell>0$, 
then $\frac{1}{f(x)} \to \frac{1}{\ell}$.
If $\ell=0$, we cannot conclude anything since the additive properties 
at $-\infty$ reflect in the multiplicative properties 
at $0^+$, not at $0$. 
Thus, the results on the limit of the quotient descend from the results 
on the limit of the product with the reciprocal.
\end{proof}


If we need to calculate a limit involving an exponentiation 
$f(x)^{g(x)}$ with variable base and exponent, it will be convenient 
to write
\[
 f(x)^{g(x)} = a^{g(x)\cdot \log_a f(x)}
\]
to apply the previous theorem to the product $g(x)\cdot \log_a f(x)$.
...
\begin{example}
  ...
  For $x\to +\infty$, by the theorem on the limit of
  the product, we know that $x^2\to +\infty$ and
  $x^3\to +\infty$.
  By the theorem on the sum of limits,
  we know that $2+x^2\to +\infty$, but
  a priori, we cannot apply this theorem
  to the limit of $(2+x^2)-x^3$ since
  $(+\infty)-(+\infty)$ is an indeterminate form
  (it does not fall under the hypotheses of that theorem).
  We must then intuit that the powers of $x$
  with a higher exponent are predominant and should
  be highlighted through
  algebraic manipulations. The expression
  for which we want to calculate the limit
  can thus be rewritten as follows:
  \begin{align*}
  \frac{2+x^2-x^3}{(x^2-x+2)^2}
    &= \frac{x^3 \enclose{\frac 2 {x^3}+\frac{1}{x}-1}}{x^4\enclose{1-\frac 1 x
  + \frac{2}{x^2}}^2}
    = \frac{1}{x}\cdot \frac{\frac 2 {x^3}+\frac{1}{x}-1}{\enclose{1-\frac 1 x +
  \frac{2}{x^2}}^2}.
  \end{align*}
  At this point, the expression no longer presents indeterminate forms.
  Applying the previous theorems, we can then assert that
  \begin{align*}
  \lim_{x\to+\infty}\frac{1}{x}\cdot \frac{\frac 2 {x^3}+\frac{1}{x}-1}{\enclose{1-\frac 1 x + \frac{2}{x^2}}^2}
    &= \frac{1}{+\infty} \cdot \frac{\frac 2
  {(+\infty)^3}+\frac{1}{+\infty}-1}{\enclose{1-\frac 1 {+\infty} +
  \frac{2}{(+\infty)^2}}^2}\\
  &= 0 \cdot \frac{0+0-1}{(1-0+0)^2} = 0\cdot (-1) = 0.
  \end{align*}
\end{proof}

Defining operations on infinite quantities
is indeed convenient in limit calculations.
However, one must be aware that $\bar \RR$ is not a field,
and thus operations with the symbols $+\infty$ and $-\infty$
do not adhere to many of the rules we are accustomed to
with finite numbers.
It will therefore be appropriate to immediately reduce to an expression
that makes sense in $\RR$, on which we can
apply algebraic manipulations with more confidence.
For this reason, in many texts, the intermediate expression involving 
operations performed on the quantities $+\infty$ and $-\infty$ is not considered
valid,
and it is preferred to write the result directly.
  
\begin{example}
Calculate the following limit in the complex field:
\[
\lim_{z\to 0} \enclose{\frac 1 {z} - \frac 1 {z^2-z}}. 
\]
\end{example}
\begin{proof}
  Again, we cannot directly apply the limit calculation rules 
  as we would obtain the indeterminate form 
  $\infty - \infty$.
  However, we can perform a simple algebraic manipulation
...
\begin{comment} % we do not yet have the tools to do these exercises
\begin{exercise}
  Calculate 
  \[
  \lim_{x\to 0^+} x^x.
...
\begin{exercise}
  Find an example of functions $f(x)$ and $g(x)$ such that 
  \[
     \lim_{x\to 0} f(x) = 0, \qquad 
     \lim_{x\to 0} g(x) = 0
  \]
...
\subsection{frequent and definitive properties}

\begin{definition}[frequent and definitive properties]
We will say that a predicate $P(x)$ defined on a set $A\subset \RR$ 
of which $x_0$ is an accumulation point holds 
\emph{definitively}%
\mymargin{definitively}%
\index{definitively} for $x\to x_0$ if
it holds in a neighborhood of $x_0$, that is:
\[
  \exists U\in \B_{x_0}\colon \forall x \in A\cap U\setminus\ENCLOSE{x_0}\colon P(x).
\]
We will say that $P(x)$ holds \emph{frequently}%
\mymargin{frequently}%
\index{frequently} for $x\to x_0$
if in every neighborhood of $x_0$ there is at least one point $x\neq x_0$ 
where it holds:
\[
...
Clearly, if a property holds definitively, it also holds frequently,
indeed if there is a neighborhood $U$ on which the property holds, for any other
neighborhood
$V$, the property holds on $U\cap V$, which is never empty.
If a property holds frequently, it means, in particular, that it holds for 
infinitely many different values, since if it holds at a point $x\neq x_0$, 
we can always find a neighborhood $V$ of $x_0$ that does not contain $x$,
and in that neighborhood, we find another point where the property holds. 
Iterating the process, we can find infinitely many different points where 
the property is valid.

The two properties are complementary in the sense that
based on the properties of logical quantifiers, the following relation holds:
\[
  \text{not frequently $P(x)$} \iff
  \text{definitively not $P(x)$}
\]

If two properties $P(x)$ and $Q(x)$ hold definitively, then
$P(x)\land Q(x)$ also holds definitively. If instead, they both
hold frequently, then $P(x) \lor Q(x)$ holds frequently.

\begin{example}
The property $x^3 - x > 1000 x^2 + 1$
holds definitively for $x\to +\infty$.
The property $x>0$ holds frequently for $x\to 0$.
\end{example}


The definition of limit $f(x) \to \ell$ for $x\to x_0$ 
could thus be stated as follows:
for every neighborhood $U$ of $\ell$, we have $f(x)\in U$ definitively.
And its negation is: there exists a neighborhood $U$ of $\ell$ for which
frequently $f(x)\not\in U$.
...
\label{th:confronto}%
\index{theorem!of comparison}%
\index{limit!comparison}%
\index{theorem!of the two policemen}%
\index{policemen!theorem of the}%
\mymark{***}%
Let 
$f$, $g$, $h$ be three real functions%
\mynote{It is not possible to make comparisons between complex values, since
there is no ordering on complex numbers}
defined on the same domain $A\subset \RR$ 
with accumulation point $x_0$
\mymargin{comparison between limits}%
\index{comparison between limits}
\begin{enumerate}
\item
If for every $x\in A$ we have
\[
f(x) \le g(x)
\]
and if both functions admit a limit: $f(x) \to \ell_1$ 
and $g(x) \to \ell_2$ for $x\to x_0$
then
\[
\ell_1 \le \ell_2.
\]
...
f(x) \le g(x)
\]
and if $f(x)\to +\infty$, then also $g(x) \to +\infty$ for $x\to x_0$.
Conversely, if $f(x) \le g(x)$ and $g(x) \to -\infty$, then also $f(x) \to
-\infty$.

\item
(the theorem of the policemen)
If for every $x\in A$ it holds
\mymargin{theorem of the policemen}%
\index{theorem!of the policemen}%
\[
f(x) \le h(x) \le g(x)
\]
 and if the two
functions $f$ and $g$ have the same limit: $f(x) \to \ell$ and $g(x)\to \ell$
for $x\to x_0$
then also $h(x) \to \ell$.
\end{enumerate}
\end{theorem}
...
\begin{enumerate}
\item
If, by contradiction, $\ell_1 > \ell_2$,
the function $f(x)-g(x)$ would have a positive limit, and by the theorem
of sign permanence, it should be definitively positive. 
But this contradicts the hypothesis $f(x)-g(x)\le 0$. 

\item 
If $g(x)\not\to +\infty$, it means that 
there exists $\alpha \in \RR$ such that $g(x)<\alpha$ frequently. 
But since definitively $f(x)>\alpha$, we find 
that frequently $f(x)>g(x)$, in contrast with the hypothesis 
$f(x)\le g(x)$.

\item
If $f$ and $g$ have the same limit $\ell$, it means that for every
$U$ neighborhood of $\ell$, we have definitively $f(x)\in U$ and $g(x)\in U$.
Since $U$ is an interval, $h(x)$ is also definitively in $U$, 
and thus $h(x)\to \ell$.
\end{enumerate}
\end{proof}
...
\end{example}
\begin{proof}[Solution.]
  We know that $\lfloor x\rfloor \le x$, thus 
  \[
    2x - \lfloor x \rfloor \ge 2x -x = x \to +\infty.  
  \]
  Therefore, by comparison, we obtain the required result.
\end{proof}

\begin{corollary}[bounded times infinitesimal]%
\label{cor:limitata_per_infinitesima}%
\mymark{**}%
  If $f(x)$ is a bounded function and $g(x)\to 0$ for 
  $x\to x_0$, then also
  \[
    f(x)\cdot g(x) \to 0 \qquad \text{for $x\to x_0.$} 
  \]
\end{corollary}
%
\begin{proof}
  If $f$ is bounded, it means there exists $R>0$ such that $\abs{f(x)}\le R$
  ...
  \]
  By comparison, we deduce that $\abs{f(x)\cdot g(x)}\to 0$, 
