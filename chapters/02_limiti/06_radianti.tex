...
Se scegliamo due diverse misure $\tau$ e $T$ pe l'angolo giro, 
la relazione tra le due funzioni è semplicemente:
\begin{equation}\label{cambio_angolo}
  \sin_\tau \frac{x}{\tau} = \sin_1 x = \sin_T \frac{x}{T}
\end{equation}
in quanto se $x$ è la misura di un angolo quando l'angolo giro 
è $\tau$ allora $\frac x \tau$ rappresenta lo stesso angolo 
quando l'angolo giro ha misura $1$.

\begin{theorem}[limite notevole $\sin$]
Per ogni $0 \le x \le \frac \tau 2$ la successione
\[
...
e $t>0$ è tale che $0\le nt <\frac \tau 4$ allora si ha
\begin{equation}\label{eq:4813509}
  \sin n t \le n\sin t\le \tg nt.
\end{equation}
Possiamo dimostrare per induzione l'implicazione 
$nt \le \frac \tau 4 \implies$~\eqref{eq:4813509}.
Se $n=1$ è ovvio.
...
\begin{align*}
  \sin (n+1)t 
  &= \sin nt \cdot \cos t + \cos nt\sin t
  \le \sin nt + \sin t
\end{align*}
e usando l'ipotesi induttiva $\sin nt\le n\sin t$ 
si ottiene 
\[
 \sin(n+1)t \le n\sin t+\sin t = (n+1)\sin t  
\]
...
Mostriamo innanzitutto che $a_n\le a_{n+1}$.
Posto $t = \frac{x}{n(n+1)}$ si tratta di verificare 
che 
\[
  n \sin (n+1)t \le (n+1) \sin nt
\] 
Ma si ha 
\[
  n \sin (n+1) t = n\sin nt \cos nt + n \sin t \cos nt
  \le n\sin nt + n \sin t \cos nt. 
\]
Per concludere basta dimostrare che 
$n\sin t \cos nt \le \sin nt$.
Se $nt<\frac \tau 4$ l'abbiamo dimostrato nel passo precedente. 
Altrimenti, se $\frac \tau 4 \le nt \le \tau$ 
la disuguaglianza è ovvia in quanto il lato sinistro 
è $\ge 0$ mentre il lato destro è $\le 0$.

\emph{Passo 3: il limite è finito.}
Se $x\le \tau$ allora $\frac{x}{3}<\frac \tau 4$ e quindi 
possiamo applicare il Passo 1 cont $t=\frac x 3$ per 
ottenere 
\[
  a_{3n} = 2n \sin \frac{x}{3n} \le \tg \frac x 3.
\]
Dunque la successione non può tendere a infinito.
\end{proof}

Diamo una interpretazione geometrica del risultato precedente.
Vogliamo calcolare la lunghezza di un arco di misura $2x$
sulla circonferenza unitaria. 
Dividiamo l'arco in $n$ parti uguali e consideriamo 
la poligonale formata dalle corde tese sugli angoli di misura 
$\frac {2x} n$.

Consideriamo la poligonale 
ottenuta da $n$ corde di uguale lunghezza tese sugli $ 
Se prendiamo un arco di misura $2x$ sulla circonferenza 
unitaria

La definizione classica della costante $\pi$ è il rapporto 
tra la lunghezza di una circonferenza e il suo diametro.
Ovvero, è la lunghezza di una semicirconferenza di raggio 
unitario. 


Osserviamo che $2\sin \frac{x}{2n}$ 
è, dal punto di vista geometrico, 
la lunghezza della corda 
corrispondente all'angolo $\frac{x}{n}$ sulla circonferenza 
unitaria.
Dunque $2n\sin\frac{x}{2n}$ corrisponde alla lunghezza 
della poligonale ottenuta dividendo l'angolo $x$ in $n$ parti uguali.
Dal punto di vista geometrico $\pi$ viene definito 
come la lunghezza della semicirconferenza unitaria, 
ovvero dell'arco di misura $\frac \tau 2$. 
Dunque $\pi$
il limite per $n\to+\infty$ di $4n \sin \frac{\tau}{4n}$.
E' esattamente quello che imponiamo con la seguente.
...
\[
 \pi 
  \defeq \lim_{n\to +\infty} 4n\cdot \sin_\tau \enclose{\frac \tau {4n}}.
\]
