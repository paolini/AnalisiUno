...
Se scegliamo due diverse misure $\tau$ e $T$ per l'angolo giro, 
la relazione tra le due funzioni è semplicemente:
\begin{equation}\label{cambio_angolo}
  \sin_\tau \tau x = \sin_1 x = \sin_\sigma \sigma x
\end{equation}
in quanto se $\phi(x)$ è l'omomorfismo con $\phi(\tau) = 1$ 
allora $\phi(\tau x) = 1$ per $x=1$.

in quanto $\sin_\tau \tau x$ soddisfa le stesse proprietà
che definiscono $\sin_1 x$ e $\sin_\sigma \sigma x$.
...
\emph{Passo 1:}
vogliamo dimostrare che se $n\in \NN$
...
\ge \tg nt + \tg t \\
 & \stackrel{\eqref{eq:4813509}}\ge
 n\sin t + \tg t \ge n\sin t + \sin t\\
 &=(n+1)\sin t. 
\end{align*}
Ovviamente se $(n+1)t<\frac \tau 4$ anche $nt < \frac \tau 4$ perché 
$t>0$.

\emph{Passo 2:}
dimostriamo che se $n\ge 2$ allora risulta 
\[
  a_{n+1} \ge a_n.
\]
Infatti posto $t = \frac{x}{n(n+1)}$ si ha:
\begin{align*}
  a_n &= n \sin\frac{x}{n+1} 
    = n \sin (n+1)t
    = n \sin nt \cos t + n \sin t \cos nt\\
    \stackrel{\eqref{eq:4813509}}\le 
    n \sin nt \cos t + \tg nt \cos nt
    \le n \sin nt + \sin nt \\
...
\begin{align*}
 ...
 \stackrel{\eqref{eq:4813509}}\le 3\tg \frac x 3.
\end{align*}
Per poter applicare \eqref{eq:4813509}
deve essere $\frac x 3 < \frac{\tau }{2}$, vero essendo $x\le \frac {\tau} 2$.

\emph{Passo 4: conclusione.}
Per il passo 2 la successione $a_{n+1}$ è crescente,
dunque per il Teorema~\ref{th:limite_monotona} la successione 
...
Scegliendo $\tau = 360$ si trova $\ge a_4 = 4 \sin \frac{180^o}{4} = 2 \sqrt 2 > 2.8$ da cui,
per la proprietà di monotonia di $a_n$, si trova $\pi > 2.8$.
Osservando che $a_{2^n} = 2^n \sin \frac{45^o}{2^{n-2}}\le 4 \tg 45^o = 4$
si ottiene $\pi<4$.
...
\begin{equation}\label{eq:771743}
   \lim_{t\to+\infty} t \sin_\tau \frac{\tau}{2t}.
\end{equation}
Infatti 
\[
...
\]
per la definizione di $\pi$, entrambi i lati tendono a $\pi$
