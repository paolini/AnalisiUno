...
\mynote{prodotto scalare definito positivo}%
...
\mynote{disuguaglianza di Young}%
...
\mynote{disuguaglianza di Cauchy-Schwarz}%
...
\mynote{proprietà del parallelogramma}%
...
\mynote{derivato}\index{derivato} di $A$ l'insieme dei punti di accumulazione (che si potrebbe denotare con $A'$);
\item
$x$ è \emph{punto isolato}%
\mynote{punto isolato}\index{punto!isolato} di $A$ se è un punto di $A$ ma non di accumulazione per $A$ cioè se esiste $r>0$ per cui
...
\mynote{spazio topologico}\index{spazio topologico}, che $\tau$ è una
\emph{topologia}%
\mynote{topologia}\index{topologia} e che gli elementi di $\tau$ sono \emph{aperti}%
\mynote{aperti}\index{aperti}
se:
\begin{enumerate}
...
\mynote{separato}\index{separato} (o $T_2$ o di Hausdorff)
...
\mynote{chiuso}\index{chiuso} se il complementare
è aperto ($X\setminus A\in \tau$).
Diremo che $A\subset X$ è un \emph{intorno}%
\mynote{intorno}\index{intorno} del punto $a\in X$ se esiste $U\in \tau$
...
\mynote{base di intorni}\index{base di intorni} di $x$ se valgono le seguenti proprietà:
\begin{enumerate}
...
\mynote{sequenzialmente continua}\index{sequenzialmente!continua} nel punto $x\in X$ se
...
\mynote{continua}\index{continua} in un punto $x\in X$ se
...
\emph{limitato}%
\mynote{limitato}\index{limitato} se è contenuto in una palla ovvero se
...
\mynote{sequenzialmente compatto}\index{sequenzialmente!compatto} se da ogni
...
\mynote{compatto}\index{compatto}
...
\mynote{uniformemente continua}\index{uniformemente continua} se 
  \[
...
\mynote{completo}\index{completo}
...
\mynote{norma uniforme}\index{norma!uniforme} (o norma del $\sup$)
di $f$ come
\[
...
\begin{theorem}[convergenza dominata localmente uniforme]%
  ...
  \mynote{convergenza dominata uniforme}%
...
\begin{theorem}[passaggio della derivata sotto il segno di integrale]%
  ...
  \mynote{scambio della derivata con l'integrale}%
...
\mynote{trucco di Feynman}\index{trucco!di Feynman}:
...
\emph{somma}%
\mynote{somma}\index{somma} della serie
\[
...
\mynote{integrala di una serie}%
\index{integrala!di una serie}%
...
\mynote{Gregory-Leibniz}\index{Gregory-Leibniz}
...
\mynote{sistema ortonormale}\index{sistema ortonormale} se $\langle e_k, e_j\rangle = 0$ quando
$k\neq j$ e $\langle e_k, e_k\rangle = 1$ per ogni $k\in \NN$.
Diremo che $e_k$ è una \emph{base hilbertiana}%
\mynote{base hilbertiana}\index{base!hilbertiana} se è un sistema ortonormale
...
\mynote{disuguaglianza di Bessel}\index{disuguaglianza!di Bessel}:
...
\mynote{formula di Werner}\index{formula di Werner}:
...
\mynote{polinomio trigonometrico}\index{polinomio trigonometrico}
...
\mynote{coefficienti di Fourier}\index{coefficienti di Fourier} di $f$
e la serie
\[
  \sum_{k=0}^{+\infty} c_k e_k
\]
si chiama \emph{serie di Fourier}%
\mynote{serie di Fourier}\index{serie!di Fourier} di $f$.
Il polinomio trigonometrico
\[
  f_N = \sum_{k=0}^{N} c_k e_k
\]
si chiama \emph{sviluppo di Fourier}%
\mynote{sviluppo di Fourier}\index{sviluppo di Fourier} di ordine $N$ per $f$.
\end{definition}
...
il seguente\footnote{vedi note storiche a pag~\pageref{note:Basilea}}.
...
\mynote{insieme di Cantor}\index{insieme!di Cantor}.
