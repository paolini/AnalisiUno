...
è limitata (superiormente / inferiormente) se
...
Anche nel caso complesso: se $a_n\in \CC$ è una successione convergente allora è
limitata.
\end{theorem}
%
\begin{proof}
...
Entrambi i risultati sono validi per successioni reali e complesse.
\end{theorem}
%
\begin{proof}
...
$\big\lvert\abs{x}\big\rvert = \abs{x}$ la definizione di limite $a_n\to 0$ è
equivalente a $\abs{a_n-0}\to 0$.
\end{proof}
...
\begin{definition}[continuità sequenziale su $\RR$]
Sia $f\colon A \subset \RR \to \RR$ una funzione. Diremo che
$f$ è \myemph{sequenzialmente continua in un punto} $a\in A$ se per ogni
successione convergente $a_n \to a$ con $a_n\in A$
 si ha
 \[
   f(a_n) \to f(a).
 \]

 Diremo che $f$ è \myemph{sequenzialmente continua} se è sequenzialmente
 continua in ogni punto $a\in A$.
\end{definition}
...
\begin{theorem}[equivalenza delle due definizioni di continuità]
Sia $f\colon A \subset \RR \to \RR$ una funzione e $a\in A$.
Allora $f$ è continua in $a$ se e solo se è sequenzialmente continua in $a$.
\end{theorem}
%
\begin{proof}
Supponiamo che $f$ sia continua nel punto $a$ e dimostriamo che è anche sequenzialmente
continua nello stesso punto. Sia dunque $a_n\in A$ una successione tale che
...
Viceversa supponiamo che $f$ sia sequenzialmente continua nel punto $a$ e
supponiamo, per assurdo che non sia continua in $a$. Allora negando la
...
Abbiamo quindi trovato una successione che viola la condizione di continuità
sequenziale nel punto $a$ e abbiamo dunque concluso la dimostrazione.
\end{proof}

In base al teorema precedente, nel seguito parleremo sempre
di \emph{continuità}
anche se la condizione che useremo è quella di
\emph{continuità sequenziale}.

La funzione $f(x)=x$ è ovviamente continua.
...
Basta mostrare che è sequenzialmente continua:
...
è continua.
Basta osservare che è sequenzialmente continua.
...
(si riguardi la definizione) a garantire che la funzione $f$ sia continua.
\end{example}
...
Dunque $f(x)=c$ è sequenzialmente continua.
Se $f(x)=x$ e $a_n\to a$ allora $f(a_n)=a_n \to a =f(a)$. Dunque $f(x)=x$
è sequenzialmente continua.

Se $a_n \to a$ allora per definizione di continuità sequenziale
si ha $f(a_n)\to f(a)$ e $g(a_n)\to g(a)$.
Per il teorema~\ref{th:limite_somma} si ottiene quindi che
$f(a_n)+g(a_n)\to f(a) + g(a)$ e dunque $f+g$ è sequenzialmente continua.
Per lo stesso teorema anche $f-g$ è sequenzialmente continua.
Analogamente tramite il teorema~\ref{th:limite_prodotto} si ottiene che
$f\cdot g$ è sequenzialmente continua, con il teorema~\ref{th:limite_rapporto}
si ottiene che $f/g$ è sequenzialmente continua e con il teorema
...
\begin{proposition}[proprietà fondamentale della convergenza]
...
Nel caso $a<1$ si procede in maniera analoga, la funzione risulterà strettamente
 decrescente su $\QQ$ e si potrà estendere in maniera su tutto $\RR$ mantenendo
...
\begin{proof}
\mymark{***}
Dimostriamo solamente che $f$ ha minimo, per il massimo la dimostrazione procede infatti in maniera del tutto analoga.
...
$x_0 \in [a,b]$. Essendo $f$ sequenzialmente continua si avrà dunque $f(a_{n_k}) \to f(x_0)$. Ma noi sapevamo che $f(a_n)\to m$ e dunque anche $f(a_{n_k}) \to m$. Concludiamo quindi che $f(x_0) = m$ cioè $m$, l'estremo inferiore, è un valore assunto dalla funzione ed è quindi un minimo. Dal canto suo $x_0$ è un punto di minimo assoluto.
\end{proof}
