...
Il seguente teorema, affatto banale, 
...
\begin{enumerate}
  \item[5.] densità: se $x<y$ esiste $z$ tale che $x < z < y$.
\end{enumerate}
...
Potremo quindi definiamo l'operazione di \myemph{sottrazione}:
\[
    x - y = x + (-y).
\]
...
Risulta immediato dimostrare che l'insieme $\NN$ appena definito
...
Le definizioni per induzione ci permettono di definire le iterate di una funzione.
...
se $n$ è un intero tra $n$ e $n+1$ non ci sono altri numeri interi.
...
Se $n$ non è pari diremo che è \myemph{dispari}. 
...
\mymargin{sommatoria}%
Se $a,b\in \ZZ$, $b-1 \ge a$ e 
$f\colon \ENCLOSE{x\in \ZZ\colon a\le x \le b}\to X$
è una funzione a valori in un gruppo additivo $X$ (ad esempio $\RR$)
si definisce
\[
  \sum_a^b f
...
\[
    \displaystyle \sum_a^{a-1} f = 0, \qquad
    \displaystyle \sum_{a}^{a+n} f = \enclose{\sum_a^{a+n-1} f} + f(a+n).
\]
Analogamente se $X$ è un gruppo moltiplicativo si definisce
\[
  \prod_a^b f
  \qquad\text{ovvero}\qquad
  \prod_{k=a}^b f(k)
...
ponendo:
\[
    \displaystyle \prod_{a}^{a - 1} f = 1, \qquad
    \displaystyle \prod_{a}^{a+n} f = \enclose{\prod_a^{a+n-1} f} \cdot f(a+n).
\]
\end{definition}

La definizione precedente ci permette, ad esempio, 
di definire le potenze e il fattoriale
come prodotti ripetuti:
\[
  x^n = \prod_{k=1}^n x, \qquad
  n! = \prod_{k=1}^n k.
\]

Il simbolo $\sum_{k=a}^b$ è chiamato \myemph{sommatoria} ed è importante 
imparare a manipolarlo nel modo corretto.
La variabile $k$ si chiama \emph{indice} della sommatoria ed è una variabile 
muta: significa che l'espressione non dipende da $k$, e che al posto di $k$
potremmo usare una qualunque altra variabile (sempre che non sia stata utilizzata 
altrove nella stessa proposizione).
I numeri $a$ e $b$ si chiamano \emph{estremi} della somma perché sono gli indici 
del primo e dell'ultimo termine che viene sommato.
L'operatore sommatoria è lineare nel senso che risulta:
\[
  \sum_{k=a}^b c\cdot f(k) = c\cdot \sum_{k=a}^b f(k), \qquad 
  \sum_{k=a}^b \enclose{f(k) + g(k)} = \sum_{k=a}^b f(k) + \sum_{k=a}^b g(k).
\]
ed è additivo:
\[
  \sum_{k=a}^c f(k) = \sum_{k=a}^{b-1} f(k) + \sum_{k=b}^c f(k).
\]
...
\[
\sum_{k=1}^n (k+1)^3 - \sum_{k=1}^n k^3 = \sum_{k=1}^n (3k^2+3k+1).
\]
Al lato sinistro tutti i termini delle due somme si cancellano 
a vicenda,\footnote{%
...
e il primo della seconda: dunque si ottiene $(n+1)^3 - 1$.
Per il lato destro abbiamo (utilizzando anche la formula trovata in precedenza):
\begin{align*}
...
\begin{align*}
  ...
  = (n+1)\cdot \Enclose{(n+1)^2 - 3\frac n 2 - 1} \\
  &= (n+1)\cdot \Enclose{n^2+2n - 3 \frac n 2}
  = \frac 1 2 (n+1)\cdot n \cdot (2n + 1)
\end{align*}
che è quanto volevamo dimostrare.

Con lo stesso metodo si può trovare una formula per il calcolo 
di $\sum_{k=1}^n k^3$ e così via. 
\end{proof}

L'utilizzo della variabile muta nella notazione delle sommatorie
ci permette di effettuare il \emph{cambio di variabile:}
posto $I_n = \ENCLOSE{0,1, \dots, n-1}$ (l'insieme dei primi $n$ numeri naturali) 
se $g\colon I_n \to I_n$ è bigettiva possiamo fare il cambio di variabile 
$k = g(j)$ e ottenere:
\[
...
\begin{definition}[funzione additiva]
  ...
  Una funzione $f$ definita tra due gruppi additivi si dice essere 
  \emph{additiva} se per ogni $x,y$ nel dominio di $f$ risulta
  \[
    f(x+y) = f(x) + f(y).
  \]
\end{definition}

\begin{theorem}[isomorfismi di gruppi additivi]
  \label{th:estensione_additiva}%
  Siano $R$ e $S$ due gruppi additivi, totalmente ordinati, densi e continui. 
  Scelto $u\in R$, $u>0$ e $m\in S$
  esiste una unica funzione $f\colon R \to S$ che sia 
  additiva, monotona e tale che $f(u) = m$.

  Inoltre se $m>0$ la funzione $f$ è strettamente crescente,
  se $m=0$ la funzione è costante nulla, se $m<0$ la funzione 
  è strettamente decrescente (dunque se $m\neq 0$ la funzione 
  è bigettiva). 
  
  Infine se $p\in \ZZ$, $q \in \NN\setminus\ENCLOSE{0}$ 
  si ha 
  \[
...
\begin{equation}\label{eq:49613244}
  f(x+y) = f(y)+f(x)
\end{equation}
e se $x\in \QQ$ dovrà risultare $f(x)=x$.
Ma allora, essendo $f$ crescente, possiamo facilmente verificare che $f(x)=x$ per 
ogni $x\in \RR$. Infatti per ogni $q\in \NN$ si ha 
\[
   \frac{\lfloor q x\rfloor}{q} \le x \le \frac{\lceil qx \rceil}{q} 
\]
e applicando $f$ si ottiene 
\[
  \frac{\lfloor q x\rfloor}{q} \le f(x) \le \frac{\lceil qx \rceil}{q}.
\]
Ma allora $\abs{f(x)-x}\le \frac{2}{q}$ e per l'arbitrarietà di $q\in \NN$
dovrà essere $f(x) = x$. 

Ma se $f$ è l'identità l'equazione~\eqref{eq:49613244} non è 
altro che la proprietà commutativa dell'addizione.
\end{proof}
%
...
per ogni $x\in \RR$ esiste una unica funzione $f_x\colon \RR\to\RR$
che sia additiva, monotona e tale che $f_x(1)=x$.
Se definiamo 
\[
  x\cdot y = f_x(y)  
\]
...
\end{equation}
Consideriamo la funzione $f(x) = f_a(x) + f_b(x)$.
E' immediato verificare che $f$ è una funzione additiva
in quanto $f_a$ e $f_b$ lo sono e l'addizione è commutativa.
Se $a,b\ge 0$ è anche immediato verificare che $f$ è crescente 
in quanto $f_a$ e $f_b$ lo sono. 
Siccome $f(1) = f_a(1)+f_b(1)=a+b$ per il teorema~\ref{th:estensione_additiva}
la funzione $f$ deve coincidere con $f_{a+b}$ e quindi 
\eqref{eq:295254} è verificata.
Osserviamo ora che $-f_m(x) = f_{-m}(x)$ sempre grazie all'unicità nel 
teorema~\ref{th:estensione_additiva} visto che $-f_m$ è additiva, monotona 
e $-f_m(1)=-m$.
Dunque se $a<0$ e $b<0$ ci possiamo ricondurre al caso precedente 
cambiando tutto di segno:
\[
f_{a}+f_{b} = - (f_{-a}+f_{-b}) = - f_{-a -b} = f_{a+b}. 
\]
Se $a\ge 0$, $b\le 0$ e $a+b\ge 0$ sfruttiamo $(a+b) + (-b) = a$
per ottenere 
$f_{a+b} + f_{-b} = f_a$ che, riordinando i termini, 
è l'uguaglianza desiderata.
Se $a\ge 0$, $b\le 0$ e $a+b\le 0$ sfruttiamo $-(a+b) + a = -b$
per ottenere $f_{-(a+b)} + f_a = f_{-b]}$ 
...
Fissato $y\in \RR$ consideriamo la funzione $f(x) = f_x(y)$.
Abbiamo appena dimostrato che $f$ è additiva.
Osserviamo che se $y\ge 0$ e $x\ge 0$ si ha $f(x)\ge 0$ 
in quanto $f_x$ è crescente e quindi $f(x)=f_x(y) \ge f_x(0)=0$.
...
Inoltre $f_1(y) = y$ in quanto l'identità è additiva 
e monotona e quindi per unicità coincide con $f_1$ 
in quanto $f_1(1)=1$ come l'identità.
Dunque $f(1) = f_1(y) = y$.
Dunque scopriamo che $f(x) = f_x(y) = f_y(x)$ ovvero 
$x\cdot y = y\cdot x$.

Per dimostrare la proprietà associativa fissiamo $y,z\in \RR$
e consideriamo la funzione $f(x) = f_z(f_y(x))$.
Questa funzione è additiva in quanto $f_z(f_y(a+b)) = f_z(f_y(a)+f_y(b))
=f_z(f_y(a)) + f_z(f_y(b))$. 
Inoltre è monotona in quanto composizione di funzioni monotone.
Infine $f(1) = f_z(f_y(1)) = f_z(y)$. 
Dunque per unicità si deve avere $f_z(f_y(x)) = f_{f_z(y)}(x)$.
...
se $y\neq 0$ la funzione $f_y$ è bigettiva e quindi 
esiste un unico $x$ tale che $f_y(x)=y\cdot x = 1$.

Lo zero è elemento assorbente in quanto $0 \cdot x = x\cdot 0 = f_x(0)=0$
qualunque sia $x$.
D'altra parte se $f_x(y) = 0$ se non è $x=0$ la funzione $f_x$ è iniettiva 
e quindi $y=0$ è l'unico punto in cui $f_x(y)=0$. 
...
\begin{definition}[campo]
  Diremo che $\KK$ è un campo se $\KK$ è un anello commutativo con identità 
  ...
  e si denota con $x^{-1}$.
  La moltiplicazione per l'inverso si chiama anche 
  \myemph{divisione} e si denota nel modo seguente:
  \[
    x\cdot y^{-1} = x/y = \frac{x}{y}.
  \]

  Se $K$ è anche totalmente ordinato da una relazione $\le$
  diremo che $K$ è un \myemph{campo ordinato} 
  se valgono anche le seguenti proprietà:
  \[
...
è quello ereditato da $\RR$.

Dunque fissato $a>0$ per il teorema~\ref{th:estensione_additiva}
esiste una unica funzione $f\colon \RR \to \RR_+$ additiva\footnote{%
In questo caso il termine \emph{additivo} è fuorviante in quanto la funzione 
soddisfa la proprietà $f(x+y) = f(x)\cdot f(y)$ e quindi potrebbe 
essere chiamata \emph{moltiplicative}.
In generale si chiamano \emph{omomorfismi} 
\index{omomorfismo}%
le funzioni definite tra due gruppi
che rispettano l'operazione del gruppo, qualunque essa sia.
}, 
monotona e tale che $f(1)=a$.
Denotiamo con $a^x = f(x)$ questa funzione (si legga: $a$ elevato alla $x$)
che chiamiamo \myemph{funzione esponenziale} di base $a$. 
L'operazione $a^b$ si chiama \emph{potenza}.
\index{potenza}%
Si osservi che l'additività/moltiplicatività in questo caso ci dice che vale 
la seguente regola 
\[
    a^{x+y} = a^x\cdot a^y.
\]
Sui numeri naturali tale funzione coincide con la moltiplicazione ripetuta 
$n$ volte:
$a^n=a\cdots a$.
Il teorema~\ref{th:divisibilita} di divisibilità garantisce l'esistenza della 
\myemph{radice} $n$-esima:
\[
   \sqrt[n]{x} = y \iff y^n = x
\]
e la costruzione della funzione additiva nel teorema~\ref{th:estensione_additiva}
ci dice che se $p\in \ZZ$ e $q\in \NN\setminus\ENCLOSE{0}$ si ha 
\[
  a^{\frac p q} = \sqrt[q]{a^p}.  
\]

Se $a>1$ (ricordiamo che $1$ è l'elemento neutro della moltiplicazione) la funzione 
risulta essere strettamente crescente, se $a<1$ la funzione è strettamente decrescente 
e se $a=1$ la funzione $1^x=1$ è costante.

Vogliamo infine dimostrare la proprietà delle potenze ripetute:
\[
...
Sappiamo inoltre che se $a\neq 1$ la funzione esponenziale $a^x$ è bigettiva $\RR\to \RR_+$. 
La funzione inversa $\log_a\colon \RR_+\to \RR$ si chiama \myemph{logaritmo} in base $a$ 
e si caratterizza con la seguente proprietà:
\[
  \log_a x = y \iff a^y = x.
\]
L'additività del logaritmo si esprime in questo modo:
\[
...
Possiamo osservare che $\QQ\subset \RR$ è anch'esso un campo totalmente ordinato.
Se $\QQ$ fosse anche continuo si avrebbe $\QQ = \RR$ in quanto abbiamo visto 
che esiste un unico gruppo totalmente ordinato, denso e continuo
e quindi esiste anche un unico campo ordinato e continuo.
Si pone quindi la questione di dimostrare che $\RR\setminus \QQ$ non è vuoto, 
ovvero che esistono numeri \myemph[irrazionale]{irrazionali}.

Il seguente teorema ci mostra che $\sqrt 2$ (che esiste in $\RR$, ed è l'unico 
numero positivo $x$ che risolve l'equazione $x^2=2$) 
non è elemento di $\QQ$.
...
\begin{align*}
  f\colon [0,+\infty) &\to [0,+\infty)\\
   x &\mapsto x^n
\end{align*}
è continua, strettamente crescente e bigettiva.
