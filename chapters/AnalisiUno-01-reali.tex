...
%
\begin{proof}
Mostriamo che la funzione non è continua
nel punto $x_0=0$.
Infatti se $x\neq 0$ si ha
\[
\abs{\sgn(x)-\sgn(x-0)} = \abs{\sgn(x)} = 1
\]
e quindi se scegliamo $\eps<1$ non è possibile
trovare $\delta>0$ per cui valga la condizione di discontinuità
nel punto $x_0=0$.
\end{proof}
...
\[
\forall \eps>0\colon \exist \delta>0 \colon \forall z\in A\colon
