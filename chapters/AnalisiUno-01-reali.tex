...
Se $m,n\in \NN$, risulta $m\ge n$ se e solo se $m-n\in \NN$.
Se invece $m< n$ si ha $m-n = -(n-m)$ che è l'opposto di un numero naturale.
L'insieme dei naturali e dei loro opposti si chiama insieme dei numeri 
interi e si denota con\footnote{
Da \emph{zahlen} che significa ``numeri'' in tedesco.
} $\ZZ$:
\[
  \ZZ = \NN - \NN = \NN \cup (-\NN).  
\]
Se $m,n$ sono numeri interi anche $m+n$ e $m-n$ sono numeri interi.
