...
Per induzione non è difficile dimostrare che se $n,m\in \NN$ allora $n+m\in \NN$ 
e che per ogni $n\in \NN$ si ha $n\ge 0$.
...
Se $n\in \NN$ e $x\in \RR$ si può definire induttivamente il prodotto $n x$
...
\end{cases}  
\]
(definizione del prodotto come somma ripetuta).  
Per induzione è facile dimostrare che 
dati $x,y\in \RR$ e $n,m\in \NN$
si ha la proprietà distributiva 
\[
  (n+m)(x+y) = n x + n y + m x + m y
\]
e di conseguenza $n(-x) = -(n x)$.
...
\]
Le seguenti proprietà discendono immediatamente dalle proprietà della moltiplicazione:
\[
  \frac{p}{1} = p, \qquad 
  \frac{0}{q} = 0, \qquad
...
\begin{lemma}[estensione di funzioni monotone]
...
\begin{enumerate}
    ...
    \item elemento neutro: $1\cdot x = x$;
    \item positività: se $x \ge 0$ e $y\ge 0$ allora $x\cdot y \ge 0$.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item stretta positività: se $x>0$ e $y>0$ allora $x\cdot y > 0$.
\end{enumerate}
\end{theorem}
%
\begin{proof}
Dato $u\in \RR$, $u>0$, consideriamo $u$ come nuova unità su $\RR$.
Per il teorema~\ref{th:unicitaR} esiste una unica bigezione additiva 
$f\colon \RR \to \RR$.
\end{proof}
