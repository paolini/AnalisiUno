...
\begin{enumerate}
  ...
  per cui se per ogni $a\in A$ e per ogni $b\in B$ risulta $a\le b$
...
\begin{axiom}[numeri reali]
  ...
  totale $\le$ che rendono $\RR$ un gruppo ordinato, denso, continuo 
...
e definiamo l'operazione di \myemph{sottrazione}:
\[
    x - y = x + (-y).
\]
Diremo che un numero $x\in \RR$ è \myemph{positivo} se $x>0$ 
e \myemph{negativo} se $x<0$.
...
\begin{exercise}
  Dimostrare che 
  \[
    (1-a)\cdot \sum_{k=0}^{n-1} a^k = 1-a^n
  \]
\end{exercise}
...
di addizione e moltiplicazione che coinvolgo coefficienti presi da $A$
...
\[
  (x+1)^n = \sum_{k=0}^n c_{n,k} x^k  
\]
i coefficienti $c_{n,k}$ vengono chiamati 
...
\begin{theorem}[isomorfismi di gruppi additivi]
  ...
  è strettamente decrescente. E se $p\in \ZZ$, $q \in \NN\setminus\ENCLOSE{0}$ 
  si ha 
  \[
    f\enclose{\frac{pu} q} = \frac{pm}{q}.
  \] 

  Infine se $m\neq 0$ tale funzione è bigettiva.
\end{theorem}
%
\begin{proof}
  ...
  Per prima cosa dimostriamo che se $x=\frac a b\in \QQ$ si ha 
  \[ 
    f(x) = \frac{am}{b}
  \]
...
\begin{definition}[campo]
  Un insieme $K$ su cui sono definite due operazioni di addizione $+$ e moltiplicazione $\cdot$
  si dice essere un \myemph{campo}
  se $K$ con l'addizione risulta essere 
  un gruppo abeliano con elemento neutro $0$ mentre $K\setminus\ENCLOSE{0}$ è un gruppo abeliano 
  rispetto alla moltiplicazione con elemento neutro $1 \neq 0$ e infine 
...
soddisfa la proprietà $f(x+y) = f(x)\cdot f(y)$ e quindi in corrispondenza 
di una addizione sul dominio otteniamo una moltiplicazione sul codominio: 
più propriamente queste funzioni andrebbero chiamate \emph{moltiplicative}.
}, 
...
L'operazione $a^b$ si chiama \myemph{potenza}.
