...
\begin{figure}
  ...
  \caption{
    ...
    rappresenta il numero presente nella casella di coordinate $(n,m)$
...
\begin{align*}
  ...
  \QQ
  \mymargin{$\QQ$}
    &= \frac{\ZZ}{\ZZ\setminus\{0\}}
    = \left\{\frac p q \colon p\in \ZZ, q\in \ZZ, q\neq 0\right\}.
\end{align*}
...
\begin{definition}[iterata]
  ...
  \[
    f^0(x) = x, \qquad f^1(x) = f(x), \qquad
    f^2(x) = f(f(x)), \qquad f^3(x) = f(f(f(x))), \dots
  \]
...
\]
si può verificare che $A$ e $B$ sono non vuoti, $A \le B$,
...
definire:
\begin{align*}
    [\vec a,\vec b] &= \{(1-t)\vec a + t \vec b\colon t\in [0,1]\},\\
    [\vec a,\vec b) &= \{(1-t)\vec a + t \vec b\colon t\in [0,1)\},\\
    (\vec a,\vec b] &= \{(1-t)\vec a + t \vec b\colon t\in (0,1]\},\\
    (\vec a,\vec b) &= \{(1-t)\vec a + t \vec b\colon t\in (0,1)\}.
\end{align*}
