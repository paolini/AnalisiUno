...
sempre.
Se, ad esempio, non esistese il massimo di $A$ significa che scelto
...
per valori esterni alle soluzioni dell'equazione
(per tutti i valori se le soluzioni non esistono) ed
è negativa per valori interni alle due soluzioni.
...
sostanzialmente tutte le operazioni elementari mantengono la
...
Se $f\colon A \to B$ e $g\colon B\to C$ definiamo
la \myemph{funzione composta}
% era già stata definita in astratto ma meglio ripetere
...
%
\begin{proof}
Mostriamo prima di tutto la continuità della composizione.
...
\]
che non è altro che la condizione di continuità
~\eqref{eq:continuita} per $f\circ g$.
...
per il teorema~\ref{th:continuita_composizione}.
