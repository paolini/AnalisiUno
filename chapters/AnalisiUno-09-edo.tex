...
Posto $B=\ENCLOSE{\vec y\in \RR^n \colon \abs{\vec y - \vec y_0}\le R}\subset \RR^n$ 
sia $\vec f\colon [a,b]\to B$ una funzione con le proprietà:
\begin{enumerate}
  \item $\vec f$ è continua,
  \item esiste $L>0$ tale che per ogni $x\in [a,b]$, ed ogni $\vec y_1,\vec y_2 \in B$ 
  si ha 
  \begin{equation}\label{eq:hp_lipschitz}
  \abs{\vec f(x, \vec y_2) - f(x,\vec y_1)} \le L \abs{\vec y_2- \vec y_1}.  
  \end{equation}
\end{enumerate}

Allora esiste $\delta>0$ tale che scelto comunque un intervallo $I$ con 
$x_0\in I \subset [a,b]\cap [x_0-\delta, x_0+\delta]$ 
esiste una unica funzione derivabile $\vec u\colon I \to B$ tale che 
\begin{equation}\label{eq:pb_cauchy}
\begin{cases}
  \vec u'(x) = \vec f(x,\vec u(x)), \qquad \forall x\in I\\
  \vec u(x_0) = \vec y_0.
\end{cases}  
...
\begin{proof}
  \emph{Passo 1.}
  Per prima cosa vogliamo dimostrare che una funzione $\vec u\colon I\to B\subset \RR^n$
  è derivabile e risolve \eqref{eq:pb_cauchy} 
  se e solo se 
  $\vec u$ è continua e risolve 
  \begin{equation}\label{eq:pb_cauchy_integrale}
  \vec u(x) = \vec y_0 + \int_{x_0}^x \vec f(t,\vec u(t))\, dt.
  \end{equation}

  Infatti se $\vec u$ è derivabile e 
  $\vec u'(x) = \vec f(x, \vec u(x))$ per ogni $x\in[a,b]$ allora 
  siccome $\vec f$ è continua la funzione $\vec f(t,\vec u(t))$ è continua 
  e quidi $\vec u'$ è di classe $C^1$. Posso allora integrare ambo i membri 
  è ottenere:
  \[
  ...
  la validità di~\eqref{eq:pb_cauchy_integrale}.

  Viceversa se $\vec u$ è continua e soddisfa~\eqref{eq:pb_cauchy_integrale}
  allora, sempre per il teorema fondamentale del calcolo possiamo affermare
  che l'integrale nel lato destro nell'uguaglianza~\eqref{eq:pb_cauchy_integrale}
  ...
  il problema di Cauchy~\eqref{eq:pb_cauchy}.

  \emph{Passo 2.}
  Visto che $[a,b]\times B$ è un insieme chiuso e limitato in $\RR \times \RR^n$ 
  il teorema di Weierstrass garantisce che esiste $M>0$ tale che 
  $f(x,y)\le M$ per ogni $x\in [a,b]$ e ogni $y\in B$.
  ...
  Consideriamo ora l'insieme $X=C(I,B)$ di tutte le funzioni continue $\vec u\colon I\to B\subset \RR^n$
  ...
  \]
  Osserviamo che l'equazione~\eqref{eq:pb_cauchy_integrale} può essere riscritto
  come equazione di punto fisso: $T(\vec u) = \vec u$. 
  Il nostro intento è quindi quello di utilizzare il teorema di Banach-Caccioppoli (delle contrazioni)

  \emph{Passo 3.}
  Dimostriamo ora che $T(X)\subset X$ cosicché possiamo pensare a $T$ come ad un operatore 
  ...
  \end{align*}
Visto che abbiamo scelto $\delta < \frac{R}{M}$ si ottiene che $v(x) \in B$ per ogni $x\in [a,b]$ 
e quindi effettivamente $T(u)\in B$.
...
\abs{T(\vec u)(x)-T(\vec v)(x)} 
  ...
  &\le \abs{\int_{x_0}^x L \abs{\vec u(t), \vec v(t)}} 
  \le \abs{\int_{x_0}^x L d_\infty(\vec u, \vec v)}\, dt  \\
...
Ma abbiamo già osservato che $T(\vec u) = \vec u$ è equivalente al problema di Cauchy~\eqref{eq:pb_cauchy}
...
per ogni $(x_0,\vec y_0)\in \Omega$ esiste $\rho>0$ ed esiste $L>0$
tali che la condizione~\eqref{eq:hp_lipschitz} è verificata 
per ogni $x\in [x_0-\rho, x_0+\rho]$ e per ogni $\vec y$ tale 
che $\abs{\vec y-\vec y_0}\le \rho$.

A parole potremmo dire: $\vec f(x,\vec y)$ è continua e 
...
Allora presi $\vec y, \vec z \in \RR^n$ con $\vec y, vec z)\in K$ si può scomporre l'incremento vettoriale 
...
\myqrshortdoclink{edo_463}{Studio qualitativo esempio~\ref{ex:edo_463}}}
\begin{figure}
...
\begin{proof}[Svolgimento.]
In questo caso si ha $f(x,y)=(y^2-1)x\sin x$, che \`e una funzione
di classe $\mathcal C^1$ su tutto $\Omega=\RR^2$.
