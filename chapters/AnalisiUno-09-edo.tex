...
Innanzitutto supporremo sempre che le nostre equazioni siano \myemph{senza memoria}
cioè che la funzione $u$ e le sue derivate $u'$, $u''$\dots,
vengano tutte calcolate nello stesso punto $x$. In caso contrario si parla di
equazioni \emph{con memoria}, argomento che non toccheremo.
...
\begin{equation}\label{eq:375456}
  \vec F(x, \vec u(x), \vec u'(x), \dots, \vec u^{n}(x)) = 0
\end{equation}
...
di ordine $n$ può essere ricondotta ad un sistema di $n+1$ equazioni
...
al sistema di $n+1$ equazioni del primo ordine nella variabile
$\vec u = (u_1, \dots, u_{n+1})$
\[
  \begin{cases}
    F(x, u_1(x), u_2(x), \dots, u_{n+1}(x)) = 0\\
    u_2(x) = u_1'(x)\\
    u_3(x) = u_2'(x)\\
    \quad \vdots \\
    u_{n+1}(x) = u_n'(x)
...
\]
si aggiunge una \myemph{condizione iniziale} ovvero scelto un
...
\[
  u'(x) = F(x, u(x))
\]
dove nella funzione $F$ risulta possibile separare le variabili
...
\begin{figure}
\online{http://paolini.github.io/recurrence/ode.html?r=5&x=0&y=0&points=0_0_0_0.5_0_1_0_1.5_0_2_0_-.5_0_-1_0_-1.5_0_-2_2_0_2.5_0_3_0_3.5_0_4_0_1.8_0_-1.75_0_-2_0_-2.5_0_-3_0_-3.5_0_-4_0&f=x*y\%5E2\%2Bx}{i grafici delle soluzioni dell'equazione differenziale~\eqref{eq:43856}}
\centering
...
%
\begin{figure}
\online{http://paolini.github.io/recurrence/ode.html?r=7&x=3.3&y=0.2&points=0.4_-0.03_0.7_0.05_0.9_-0.05_1.25_0.05_1.6_-0.06_1.8_0.06_2_-0.05_2.4_0.05_2.6_-0.03_2.9_0.03_3.3_-0.04_3.6_0.05_3.9_-0.03_4.2_0.02_4.4_-0.04_4.7_0.05_5.1_-0.05_5.3_0.05_5.7_-0.06_5.95_0.09_6.31_-0.1_6.5_0.07_7_-0.07_7.3_0.07_7.7_-0.05_8_0.06_8.4_-0.07_8.7_0.03_9.1_-0.06_9.4_0.07_9.8_-0.06_9.9_0.08_0.1_0.02&f=abs(y)\%5E(4\%2F3)\%2Fy}{Il baffo di Peano formato dalle soluzioni del problema di Cauchy~\eqref{eq:94467}}
\centering
...
\begin{exercise}[serie binomiale]
\label{ex:serie_binomiale}%
\mymargin{serie!binomiale}\index{binomiale!serie}
Per ogni $\alpha\in \RR$ per ogni $x\in (-1,1)$ si ha
\[
  (1+x)^{\alpha}  = \sum_{k=0}^{+\infty} {\alpha \choose k} x^k.
\]
Di conseguenza la funzione $x^\alpha$ è analitica.
\end{exercise}
%
\begin{proof}
...
Dunque la funzione
\[
 u(x) = \sum_{k=0}^{+\infty} {\alpha \choose k} x^k
\]
è definita per ogni $x\in(-1,1)$.
All'interno del raggio di convergenza possiamo quindi derivare
la serie termine a termine per trovare:
\[
  u'(x) = \sum_{k=1}^{\infty} k{\alpha \choose k} x^{k-1}.
\]
...
&= \sum_{k=0}^{+\infty} \Enclose{\frac{\alpha(\alpha-1) \cdots (\alpha-k)}{k!}
 ...
 &= \alpha \sum_{k=0}^{+\infty} {\alpha \choose k} x^k = \alpha \cdot u(x).
\end{align*}
Dunque $u(x)$ risolve una equazione a variabili separabili:
\[
  (1+x) u'(x) = \alpha \cdot u(x).
\]
Separiamo le variabili:
\[
\frac{u'(x)}{u(x)} = \frac{\alpha}{1+x}
\]
integriamo
\[
 \ln u(x) = \alpha \ln (1+x) + c
\]
e osservando che $u(0) = 1$ troviamo che $c=0$ e quindi
\[
 u(x) = e^{\alpha \ln(1+x)} = (1+x)^\alpha
\]
come volevamo dimostrare.

Dunque la funzione $(1+x)^\alpha$ è sviluppabile in serie di potenze
per $x\in (-1,1)$. Più in generale osserviamo che se $\abs{x-x_0}<x_0$ si ha
\begin{align*}
  x^\alpha
  &= (x_0 + x-x_0)^\alpha
...
\[
  u''(x) = -\frac{\ell}{g} \sin u(x).
\]
Siamo quindi nel caso dell'esempio precedente dove $F(u) = -\frac{\ell}{g} \sin u$
...
\[
\frac{-\ell}{g}\int_{u_0}^u \sin y\, dy = \frac{\ell}{g}\enclose{\cos u - \cos u_0}.
\]
Si ottiene dunque
\[
  u'(x) = \pm \sqrt{\frac{2\ell}{g}(\cos u(x) - \cos u_0)}.
\]
Separando le variabili e integrando tra $0$ e $x$ si ottiene
\[
\sqrt{\frac g \ell}\int_0^x \frac{u'(x)\, dx}{\pm\sqrt{2\cos u(x)-2\cos u_0}} = \int_0^x dx  = x.
\]
e cambiando variabile $u=u(x)$, $du = u'(x) dx$:
\begin{equation}\label{eq:4898944}
\sqrt{\frac g \ell}\int_0^{u(x)} \frac{du}{\pm\sqrt{2\cos u-2\cos u_0}} = x.
\end{equation}
...
\[
-\sqrt{\frac{g}{\ell}}\int_{u_0}^{0} \frac{du}{\sqrt{2\cos u - 2\cos u_0}}
= x_1.
\]
...
\[
  \frac{T}{4} = \sqrt{\frac g \ell}\int_0^{u_0} \frac{du}{\sqrt{2\cos u - 2\cos u_0}}.
\]
...
\]
Utilizzando ora la serie binomiale (esercizio~\ref{ex:serie_binomiale})
possiamo scrivere
\begin{align*}
\sqrt{\frac \ell g} \cdot\frac{T}{4}
...
\[
T
= 2\pi\sqrt{\frac g \ell}\sum_{k=0}^{+\infty} \enclose{\frac{(2k-1)!!}{(2k)!}}^2
...
serie per $k=0,1,2,3,4$ in quanto la somma dei termini per $k\ge 5$ mi darà
 ...
 \begin{align*}
 \sqrt{\frac{\ell}{g}}\cdot \frac{T}{2\pi}
 &=
...
l'esistenza e unicità della soluzione si ottiene dall'esistenza e unicità
del punto fisso di $T$.
...
$\vec u(x_0) = \vec y_0$ e, passando alle derivate, si scopre che $\vec u$ è
...
Fissato $(x_0, \vec y_0)$ consideriamo l'intorno $U$ in cui, per ipotesi,
...
\begin{equation}\label{eq:395109}
 \delta = \min\{\alpha, \frac{\beta}{M}, \frac{1}{L}\}.
\end{equation}
...
\begin{proposition}
\mymark{***}
Se $\vec f\in C^1(\Omega,\RR^n)$ allora $f$ soddisfa la condizione
di Cauchy-Lipschitz.
\end{proposition}
%
\begin{proof}
...
K=[a,b] \times \overline{B_M(\vec 0)}
 = \{(x,y)\in \RR \times \RR^n\colon x\in [a,b], \abs{y}\le M\}
\]
...
Se $f\colon \Omega\to \RR$ soddisfa le ipotesi del teorema di esistenza e unicità
per i sistemi del primo ordine (cioè $f$ è continua e lipschitziana nelle ultime
$n-1$ variabili uniformemente rispetto alla prima) allora il problema di
Cauchy~\eqref{eq:problema_cauchy_ordine_n} ammette una unica soluzione locale.
Esiste cioè un $\delta_0>0$ tale che per ogni $\delta < \delta_0$ posto
$I_\delta = [x_0-\delta,x_0+\delta]$ esiste una unica $u\in C^n(I_\delta)$ che
...
dato dalla applicazione $J(u)$ definita da~\eqref{eq:jet}.
...
sia $W = \{ v\in C^n(A)\colon$ $L v = b\}$ l'insieme di tutte le soluzioni.
...
\[
  (f\circ g)(u) = f(g(u)), \qquad (P\cdot Q)(z) = P(z)\cdot Q(z)
\]
...
\]
da cui $P(Q(D) u ) = (P\cdot Q)u$, come volevamo dimostare.
\end{proof}
...
\[
   u(x) = x^m e^{\lambda x}
\]
con $m\in \NN$, $\lambda \in \CC$ è un insieme linearmente indipendente
nello spazio vettoriale $C^\infty(\RR,\CC)$ sul campo $\CC$.
\end{theorem}
%
\begin{proof}
...
\mymargin{matrice e determinante wronksiano}%
\index{matrice!wronksiana}%
\index{determinante!wronksiano}%
\index{wronksiano}
...
continua e che $\{\vec v\colon \abs{ \vec v} \le 1\}$ è un insieme
