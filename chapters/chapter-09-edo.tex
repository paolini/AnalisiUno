...
\mynote{senza ritardo}\index{senza ritardo}
...
\mynote{principio di sovrapposizione}%
...
\mynote{condizione iniziale}\index{condizione iniziale} ovvero si fissa un
...
\mynote{problema di Cauchy}\index{problema!di Cauchy}.
...
\mynote{intervallo massimale}\index{intervallo massimale} cioè
...
\mynote{piccole oscillazioni}\index{piccole oscillazioni}.
...
\mynote{proprietà di Cauchy-Lipschitz}\index{proprietà di Cauchy-Lipschitz} se $f$ è continua e se
...
\le \sum_{j=1}^n L_j \abs{\vec z-\vec y}
...
\mynote{estensione della soluzione}\index{estensione della soluzione} $u(x)$.
...
\mynote{soluzione massimale}\index{soluzione massimale}.
\end{definition}
...
\mynote{esistenza e unicità locale}%
...
\mynote{esistenza e unicità globale}%
...
\mynote{omogenea}\index{omogenea}
...
(chiamato \emph{jet}%
\mynote{jet}\index{jet}) definito da
...
\mynote{determinante wronksiano}\index{determinante!wronksiano}.
\end{definition}
...
\emph{sella}%
\mynote{sella}\index{sella} (si veda figura).
...
\mynote{nodo stabile}\index{nodo!stabile} in cui tutte le
...
\mynote{nodo instabile}\index{nodo!instabile}.
...
chiama \emph{stella}%
\mynote{stella}\index{stella} (stabile o instabile a seconda
...
\mynote{nodo improprio}\index{nodo improprio}.
...
\mynote{fuoco stabile}\index{fuoco stabile}. Se $\alpha >0$ le orbite
divergono chiameremo la configurazione un \emph{fuoco instabile}%
\mynote{fuoco instabile}\index{fuoco instabile}.
...
Possiamo ottenere informazioni piuttosto precise andando a identificare un integrale 
