...
così come il quantificatore esistenziale
...
Una variante del quantificatore esistenziale richiede anche l'unicità:
...
\[
  \exists!x\colon P(x) \iff \exists x\colon \enclose{P(x)
  \wedge \neg \exists y\colon (y \neq x) \wedge P(y)}.
\]
...
\end{equation}
In particolare osserviamo che $(a,b)$ e $(b,a)$ sono diversi tra loro (a meno che non sia $a = b$) cioè l'ordine in cui vengono elencati i due elementi
è importante.
...
dei due insiemi $A$ e $B$. Se $R$ è una relazione e vale $x R y$ allora
...
con la direzione invertita $y\stackrel{R^{-1}} \mapsto x$.
...
Le funzioni sono le relazioni
\emph{univoche}\index{univoco}\index{relazione!univoca}
cioè quelle relazioni $f$ in
$A\times B$ che
...
\]
Le funzioni $\pi_1$ e $\pi_2$ si chiamano proiezioni sul primo e secondo fattore
di un prodotto cartesiano e ci permettono di estrarre le componenti di una coppia.
Suggerimento: si ricordi che a questo livello una funzione non è altro che un
sottoinsieme del prodotto cartesiano
\[
\pi_1 \subset (A\times B) \times A, \qquad \pi_2 \subset (A\times B)\times B.
\]
\end{exercise}
%
...
In astratto ogni funzione, essendo una relazione, ha una relazione inversa,
ma non è detto che tale relazione sia a sua volta una funzione.
Se abbiamo una funzione $f\colon A \to B$ abbiamo affinché esiste una funzione
inversa dobbiamo verificare due
differenti proprietà:
che per ogni $b\in B$ esista un elemento $a\in A$ tale che $f(a)=b$
(surgettività)
